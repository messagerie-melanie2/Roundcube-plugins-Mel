class FullscreenItem {
  constructor(parentSelector, open = true) {
    ...
    if (this.item.data('is-flex')) {
      ...
      else this.generate_flex();
    }

    this.item.prepend(
      '<span tabindex=-1 id=fullscreenreaderfocus class="sr-only">Ecran Mes Raccourcis</span>',
    );

    this.close_button = $(
      '<button class="fullscreen-close"><span class="icofont-close-line-circled"></span><span class="sr-only">Fermer les raccourcis</span></button>',
    ).appendTo(this.item);
  ...
  add_app(key, querry) {
    if (typeof querry === 'string') querry = $(querry);

    this.apps[key] = querry
      .addClass('apps')
      .data('app-id', key)
      .appendTo(this.item);
  }
  ...
  _set_accessibility() {
    ...
    this.aria_dialog = new aria.Dialog(
      'modalfullscreenitemid',
      'button-shortcut',
    );
  ...
  open() {
    if (this.is_flex()) this.item.parent().css('display', '');
    else this.item.css('display', '');
  ...
  close() {
    if (this.is_flex()) this.item.parent().css('display', 'none');
    else this.item.css('display', 'none');
  ...
  static close_if_exist() {
    if (
      window.shortcuts !== undefined &&
      window.shortcuts !== null &&
      window.shortcuts.is_open
    )
