$(document).ready(async () => {
  ...
  if (
    ...
    [EMPTY_STRING, 'index'].includes(rcmail.env.action)
  ) {
    ...
    class mel_filter {
      ...
      _start() {
        ...
        if (this.custom_action) {
          if (typeof this.custom_action === 'string') {
            ...
            switch (callback[0]) {
              case 'trigger':
                callback = ((trigger_name) => {
                  return (e) => {
                    rcmail.triggerEvent(trigger_name, {
                      filter: this,
                      target_event: $(e.target),
                    });
                  };
                })(callback[1]);
                break;
...
*/
    ...
    class mel_selector {
      ...
      toggle_selected_menu() {
        if ($('html').hasClass('touch')) {
          ...
          //Si pas sélectionné
          if (list.selection.length === 0) {
            if ($menu.hasClass('popupmenu'))
              $menu.removeClass('popupmenu').removeClass('hoverable');
          } else {
    ...
    class mel_filter_manager {
      ...
      _create_filters() {
        for (const filter of this.get_all_quick_filters_from_class()) {
          const $filter = $(filter);
          this.filters.push(
            new mel_filter($filter, $filter.data('action'), {
      ...
      get_default_filter() {
        if (!this.get_default_filter.filter) {
          let tmp = this.filters.filter((f) =>
            [1, true].includes(f.default_filter),
          );
      ...
      *_set_action_on_all_filters(action, where = null) {
        for (let index = 0, len = this.filters.length; index < len; ++index) {
          ...
          else if (where(this.filters[index]))
            yield action(this.filters[index]);
        }
      }
      ...
      *get_filters_active() {
        yield* this._set_action_on_all_filters(
          (f) => f,
          (f) => f.is_active(),
        );
      }
      ...
      static reset_extra_states() {
        $('#mel-label-tooltip-dropdown button')
          .removeClass('active')
      ...
      static get_line_priority() {
        if (this.get_line_label().length === 0) return;
      ...
      static show_priority($prio) {
        ...
        new mel_html2('button', {
          attribs: {
            class:
              'btn btn-secondary mel-button no-button-margin no-margin-button active hoverable',
          },
          contents: [
      ...
      static disable_extra_buttons() {
        return $('#message-list-filters-extra button')
       ...
       * @param {*} param2
       */
      static base_callback(
        filter,
        event,
        { before = null, after = null, on_reset = null },
      ) {
        if (before) before(filter, event);

        rcmail.triggerEvent('quick-filter.brefore', {
          filter,
          target_event: event,
        });

        if (!filter.can_be_multiple)
          mel_filter_manager.Instance.deactivate_all_filters(); //mel_filter.disable_all_quick_filters()
        ...
        const plugin = rcmail.triggerEvent('quick-filter.action', {
          filter,
          target_event: event,
        });
        ...
        if (!plugin?.abort) {
          ...
          if (
            mel_filter_manager.Instance.get_filters_active_array().length !== 0
          ) {
            ...
            if (
              $(rcmail.gui_objects.search_filter).find('#custom-option')
                .length === 0
            ) {
              $(rcmail.gui_objects.search_filter).append(
                `<option id="custom-option" value="${search}">Custom</option>`,
              );
            } else {
    ...
    Object.defineProperties(mel_filter_manager, {
      Instance: {
        get: function () {
          if (!mel_filter_manager._instance)
            mel_filter_manager._instance = new mel_filter_manager();
    ...
    rcmail.dom_element_is_visible = function ($item) {
      if (
        ...
        if ($item.hasClass('popover'))
          return rcmail.dom_element_is_visible($item);
        else return true;
      }
    ...
    rcmail.addEventListener('label.tooltip.click', (args) => {
      ...
      const labels = [...$target.parent().find('button')]
      ...
      let filter = mel_filter_manager.Instance.filters.filter(
        (x) => x.$item.attr('id') === 'quick-filter-labels',
      )[0];
      if (labels.length === 0) {
    ...
    rcmail.addEventListener('quick-filter.priority', async (args) => {
      ...
      if ($tooltip.length === 0) {
        const priorities = [
          ...
          mel_filter.priority(
            'Normale',
            'NOT HEADER X-PRIORITY 1 NOT HEADER X-PRIORITY 2 NOT HEADER X-PRIORITY 4 NOT HEADER X-PRIORITY 5',
            3,
          ),
        ...
        let html_div = new mel_html2('div', {
          attribs: {
            id: 'priority-tooltip',
            class: 'priority-tooltip btn-group-vertical',
          },
        });

        let html_button;
        for (const iterator of priorities) {
          html_button = new mel_html(
            'button',
            {
              class:
                'btn btn-secondary mel-button no-button-margin no-margin-button bckg true',
          ...
          html_button.onclick.push((e) => {
            ...
            for (let iterator of $('body .priority-tooltip button')) {
              ...
              if (iterator.data('isTarget') !== 1)
                iterator.removeClass('active');
              else iterator.data('isTarget', 0);
            }

            const priorities = [...$('body .priority-tooltip button')]
              .filter((e) => $(e).hasClass('active'))
              .map((e) => {
            ...
            if (priorities.length > 0) {
              ...
              target_event
                .addClass('prio')
                .addClass(`prio${$target.data('priority')}`);
            } else {
        ...
        target_event
          .addClass('priority-tooltip')
          .data('popup', 'priority-tooltip');
    ...
    rcmail.command_handler = function (command, ...args) {
      ...
      switch (command) {
        ...
        case 'select-all':
          let list =
            this[this.task == 'addressbook' ? 'contact_list' : 'message_list'];
          if (list.selection.length === 0) {
            $check.removeAttr('checked', 'checked')[0].checked = false;
            close_previsu();
          } else $check.attr('checked', 'checked')[0].checked = true;
    ...
    function interval_on_change(start, end) {
      ...
      if ($('#searchfilter .customdate').length === 0) {
        $('#searchfilter').append(
          $('<option>')
            .val(val)
    ...
    rcmail.addEventListener('init', () => {
      ...
      if ($('#s_interval .multidates').length === 0) {
        $('#s_interval')
          .append(
            $('<option>')
              .addClass('multidates')
    ...
    const { MelEnumerable } = await loadJsModule(
      'mel_metapage',
      'enum.js',
      '/js/lib/classes/',
    );
    ...
    window.mel_filter_observe =
      ...
      (async () => {
        ...
        $('#message-list-filters .filter-last-before-more').removeClass(
          'filter-last-before-more',
        );

        if ($('.message-list-filter-container .mel-filter-more').length !== 0) {
          $('.message-list-filter-container .mel-filter-more').hide();

          if (
            $(
              '.message-list-filter-container .mel-filter-more button',
            ).hasClass('activated')
          ) {
        ...
        const li_width = MelEnumerable.from($('#message-list-filters li')).sum({
          selector: (x) => $(x).width(),
        });
        if (li_width > layout_list_width) {
          ...
          for (const iterator of $('#message-list-filters li')) {
            ...
            if (width > layout_list_width) {
              $iterator.hide();
              if (
                $('#message-list-filters li.filter-last-before-more').length ===
                0
              ) {
          ...
          if (
            $('.message-list-filter-container .mel-filter-more').length === 0
          ) {
            let more_button = new mel_html2('button', {
              attribs: {
                class:
                  'mel-button no-button-margin no-margin-button mel-filter-more',
              },
            ...
            new mel_html2('div', {
              attribs: { class: 'mel-filter-more' },
              contents: more_button,
            }).create($('#message-list-filters').parent());
          }

          $('.message-list-filter-container .mel-filter-more').show();

          const visible_enum = MelEnumerable.from($('#message-list-filters li'))
            .aggregate($('.message-list-filter-container .mel-filter-more'))
          ...
          if (
            visible_enum
              .select(
                (x) =>
                  $(x).width() +
                  ($(x).hasClass('mel-filter-more') ? 0 : corrector),
              )
              .sum({}) > layout_list_width
          ) {
            $('#message-list-filters .filter-last-before-more').removeClass(
              'filter-last-before-more',
            );
            let last = visible_enum
              .where((x) => !$(x).hasClass('mel-filter-more'))
              .last(); //.hide();
