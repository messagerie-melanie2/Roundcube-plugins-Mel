...
export class MailFavoriteFolder extends MailModule {
  ...
  _generate_html_tree(tree, tree_html = null, level = 1) {
    ...
    for (let element of enum_tree) {
      ...
      let base = $('#mailboxlist [rel="' + element.full_id + '"]');
      if (base.length === 0) {
        base = $(
          `#mailboxlist [label="${element.name}"][level="${+level - 1}"]`,
        );
      }
      html = html.tag('bnum-folder', {
        'aria-level': level,
        'folder-id': element.full_id,
        'is-selected': false,
        'is-collapsed': this._is_collapsed_rel(rel),
        'is-virtual': level === 0,
        class: 'mailbox',
        ...
        label: element.name,
        icon: base.attr('icon'),
        unread: base.attr('unread'),
      });

      if (level !== 0) {
        html = html.attr('slot', 'folders');
      }
  ...
  _create_html_tree(tree) {
    ...
    let html = JsHtml.start
      ...
      .tag('bnum-folder', {
        'aria-level': 0,
        'is-selected': false,
        'is-collapsed': this._is_collapsed_rel('favourite'),
        'is-virtual': true,
        'folder-id': 'favourite',
        mailid: 'favourite',
        rel: 'favourite',
