(() => {
	...
	class workspace_frame {
		...
		get() {
			if (0 === (this._frame?.length ?? 0)) {
				this._frame = this.create().create(
					workspace_frame_manager.get_side_workspace(),
				);
			}
		...
		get_url() {
			return mel_metapage.Functions.url(
				PLUGIN_WORKSPACE,
				ACTION_WORKSPACE_WORKSPACE,
				{
	...
	class workspace_frame_manager {
		...
		hideListFrame() {
			...
			workspace_frame_manager
				.get_side_workspace()
				.css(CONST_CSS_DISPLAY, EMPTY_STRING);
			return this;
		}
		...
		createFrame(wsp) {
			if (this._frames.length >= workspace_frame_manager.MAX_FRAMES) {
				...
				rcmail.triggerEvent('wsp.frame.remove', frame);
				...
				while (
					0 !== $(`${CONST_JQUERY_SELECTOR_ID}${SIDE_FRAMES_NAME}${id}`).length
				) {
					++id;
