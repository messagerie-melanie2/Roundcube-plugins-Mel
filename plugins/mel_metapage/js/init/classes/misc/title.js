    ...
    update(id = null, focus = true) {
      ...
      try {
        if (id !== null) {
          if (this.is_framed)
            top.Title.set(
              top.$('iframe#${id}')[0].contentWindow.document.title,
              focus,
        ...
        } else if (this.is_framed) this.set(window.document.title, focus);
        else {
      ...
      if (focus && doFocus) this.focusHidden();
    }
