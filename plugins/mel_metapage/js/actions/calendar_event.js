$(document).ready(function () {
  ...
  function calendar_waiting_number() {
    if (!window.rcube_calendar || !rcube_calendar.get_number_waiting_events)
      return;
  ...
  if (parent.rcmail.env.calendars !== rcmail.env.calendars)
    parent.rcmail.env.calendars = rcmail.env.calendars;
  CalendarPageInit();
  rcmail.addEventListener('init', () => {
    rcmail.addEventListener('responsebefore', function (props) {
      ...
      if (props.response && props.response.action == 'event') {
        ...
        props.response &&
        (props.response.action === 'mailimportitip' ||
          props.response.action === 'itip-delegate' ||
          props.response.action === 'itip-remove')
      ) {
        let config = {
          _integrated: true,
          eval: 'always',
    ...
    $('#datepicker').prepend(
      $(
        ...
        try {
          const size = $('#datepicker .ui-datepicker')[0].getClientRects()[0]
            .height;
          if ($('#datepicker').hasClass('showed')) {
            //est affiché
            mel_metapage.Storage.set('datepicker_state', false);
            $('#datepicker')
              .css('margin-bottom', `-${size}px`)
              .removeClass('showed');
            $('#datepicker-onoff .icon-mel-chevron-down')
            ...
            $('#datepicker-onoff .icon-mel-chevron-up')
    ...
    setTimeout(() => {
      if (
        $('#datepicker').hasClass('showed') &&
        mel_metapage.Storage.get('datepicker_state') === false
      )
  ...
  (init_date_picker ? DatePickerInit() : Promise.resolve()).then(() => {
    if (init_date_picker) {
      $(
        ...
        if (
          $.inArray(event.keyCode, [13, 33, 34, 35, 36, 37, 38, 39, 40]) != -1
        )
          DatePickerInit();
      });
    ...
    const isSelected = (value) => {
      return $(
        'body.task-calendar .fc-toolbar.fc-header-toolbar .fc-left .active',
      ).html() === value
        ? 'selected'
        : '';
    };
    ...
    let selectGroupDay =
      '<optgroup label="' + rcmail.get_label('calendar.views-day') + '">';
    let selectGroupWeek =
      '<optgroup label="' + rcmail.get_label('calendar.views-week') + '">';
    let selectGroupMonth =
      '<optgroup label="' + rcmail.get_label('calendar.views-month') + '">';
    let selectGroupList =
      '<optgroup label="' + rcmail.get_label('calendar.views-list') + '">';
    $(
      'body.task-calendar .fc-toolbar.fc-header-toolbar .fc-left .fc-button-group .fc-button',
    ).each(function () {
      const value =
        '<option value="' +
        $(this).attr('class').split(/\s+/)[0] +
        '" ' +
        isSelected($(this).text()) +
        '>' +
        $(this).text() +
        '</option>';
      if ($(this).attr('class').indexOf('Day') !== -1) {
        ...
        $(this).attr('class').indexOf('agenda') !== -1 ||
        $(this).attr('class').indexOf('Week') !== -1
      ) {
...
async function DatePickerInit() {
  ...
  $('#datepicker-dp-title .ui-datepicker-month')
    .on('focusout', () => {
      $('#datepicker-dp-title .ui-datepicker-month').removeClass(
        'calendar-force-show',
      );
    })
    .on('change', () => {
  ...
  $('#datepicker-dp-title .ui-datepicker-year')
    .on('focusout', () => {
      $('#datepicker-dp-title .ui-datepicker-year').removeClass(
        'calendar-force-show',
      );
    })
    .on('change', () => {
...
function copy_calendar_url(input_id) {
  ...
  if (copyText.val()) {
    $('.ui-dialog :button').each(function () {
      $('.ui-dialog #' + $(this).attr('id')).val(
        rcmail.labels['calendar.copied_url_default'],
      );
    });

    $('.ui-dialog #button_' + input_id).val(
      rcmail.labels['calendar.copied_url'],
    );
  }
}
...
function check_time_validity(id) {
  ...
  let input_array = $(
    '.ui-dialog #appointment_range_' + appointment_id + ' .row',
  ).find('input');
  ...
  if (time_end.val()) {
    if (time_start.val() >= time_end.val()) {
      ...
      $('.ui-dialog button.mainaction.save.btn.btn-primary').prop(
        'disabled',
        true,
      );
      return;
  ...
  for (let i = 1; i <= input_array.length / 2; i++) {
    ...
    if (
      (time_start.val() > i_time_start.val() &&
        time_start.val() < i_time_end.val()) ||
      (time_end.val() > i_time_start.val() &&
        time_end.val() < i_time_end.val()) ||
      (time_start.val() <= i_time_start.val() &&
        time_end.val() >= i_time_end.val())
    ) {
      if (input_id != id) {
        ...
        time_end.addClass('border-danger');
        time_alert
          .text('Les heures se chevauchent avec une autre période')
          .show();
        $('.ui-dialog button.mainaction.save.btn.btn-primary').prop(
          'disabled',
          true,
...
function add_reason(id) {
  let input_array = $('.ui-dialog #appointment_reason .row:last-child').find(
    'input',
  );
  let start_input = input_array.attr('id').split('-');
  let new_id = parseInt(start_input[1]) + 1;

  $('.ui-dialog #appointment_reason').append(
    '<div id="row' +
      new_id +
      '" class="form-group row mt-3"><div class="col-10"><input type="text" class="form-control" id="time_reason-' +
      new_id +
      '" placeholder="Motif"></div><div class="col-1"><button class="btn btn-danger" title="Supprimer le motif de rendez-vous" id="reason_trash_' +
      new_id +
...
function checkWebconfName() {
  ...
  const text =
    val.length < 10
      ? rcmail.gettext('webconf_saloon_name_error_small', 'mel_metapage')
      : /^[0-9a-zA-Z]+$/.test(val)
        ? rcmail.gettext(
            'webconf_saloon_incorrect_format_number',
            'mel_metapage',
          )
        : rcmail.gettext('webconf_saloon_incorrect_format', 'mel_metapage');
