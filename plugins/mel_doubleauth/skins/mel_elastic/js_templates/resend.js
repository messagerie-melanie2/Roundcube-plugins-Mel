...
function help(text) {
  return new MainIconHtml('help', { title: text }, {})
...
function show_address() {
  ...
  const CHAR_AT = '@';
  const splitted =
    rcmail.env.double_authentification_adresse_recuperation.split(CHAR_AT);
...
function connect() {
  ...
  (top ?? parent ?? window).location.href = MelObject.Empty().url('login', {
    action: 'plugin.da.try_connect',
    params: { _token: val },
  });
}
...
async function send_code() {
  ...
  $('button').addClass('disabled').attr('disabled', 'disabled');
  const sent = await BnumConnector.force_connect(
    BnumConnector.connectors.settings_da_send_otp_token,
    { params: { _from_login: true } },
  );

  if (sent === -1) {
  } else {
    rcmail.display_message('Message envoyé !', 'confirmation');
    $('#mind-card').css('display', 'none');
    $('#resend-card')
      .css('display', 'block')
      .find('.card-body')
      .css('display', 'block');
...
MelHtml.extend(
  ...
  function ({
    ...
    return this.card({ id: 'mind-card', class: 'clear' })
      .card_title({ for: 'mind-card', style: 'display: none;' })
      ...
      .span({
        class: 'cursor-help border-bottom border-dark',
        title: show_address,
      })
...
MelHtml.extend(
  ...
  function ({
    ...
    return this.card({ id: 'resend-card', class: 'clear' })
      .card_title({ for: 'resend-card', style: 'display:none' })
      .css('display', ' none')
      ...
      .text(
        help_text.call(
          this,
          "Vous avez reçu un mail avec un code de confirmation sur votre adresse de récupération.\r\nIl s'agit d'un code à 6 chiffres.",
        ),
      )
      .end('label')
      .end()
...
export const page = MelHtml.write(
  ...
  function (html, ...args) {
    ...
    const jsHtml = html
      ...
      .resend_page_mind_guard({ show_address, back, next: send_code })
      .end()
      .resend_page_code({ help_text, send_code, connect })
      .css('display', 'none')
      .end()
      .end();
