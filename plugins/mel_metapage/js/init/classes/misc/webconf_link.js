class WebconfLink {
  constructor(link) {
    ...
    if (link.includes('#visio:')) {
      ...
      this.key = link
        .split('_key=')[1]
        .split('&_')[0]
        .replaceAll(rcube_calendar.old_newline_key, rcube_calendar.newline_key)
        .split(rcube_calendar.newline_key)[0];
      ...
      if (link.includes('_pass='))
        this.pass = link.split('_pass=')[1].split('&_')[0];
    }
  }
  ...
  static create(event) {
    if (
      ...
      return new IntegratedPubliWebconfLink(event.location);
...
class IntegratedWebconfLink extends WebconfLink {
  constructor(location, category) {
    super('');
    this.key = mel_metapage.Functions.webconf_url(location).split(
      rcube_calendar.newline_key.toUpperCase(),
    )[0];
    this.ariane = null;

    if (Array.isArray(category) && category.length > 0) category = category[0];

    if (!!category && category.includes('ws#'))
      this.wsp = category.replace('ws#', '');

    if (location.includes('_pass='))
      this.pass = location.split('_pass=')[1].split('&_')[0];
  }
}
