(() => {
  ...
  SynchroniseWorkspaces.integrated_functions = (func_name, args) => {
    switch (func_name) {
      ...
      case 'searchToAddressbookParent':
        ...
        break;
      case 'search':
        if (window.search_action !== undefined) search_action(...args.args);
        break;
      case 'update_cal':
        ...
        if (args.args !== undefined) {
          ...
          if (
            ...
            rcmail.commands['refreshcalendar'] === true
          ) {
      ...
      case 'update_location':
        try {
          if (args.args.length === 1) window.location.href = args.args[0];
          else {
            if (args.args.length === 3) {
              if ($(`iframe.${args.args[1]}`).length === 0) {
                if (
                  $(`iframe#${args.args[2]}`)[0].src !== args.args[0] ||
                  $(`iframe#${args.args[2]}`)[0].contentWindow.location.href !==
                    args.args[0]
                )
                  $(`iframe#${args.args[2]}`)[0].src = args.args[0];
      ...
      case 'refresh_frame':
        ...
        if (frame === 'mail') {
          if (rcmail && rcmail.refresh_list) rcmail.refresh_list();
        }
        ...
        switch (args.args[0]) {
          case 'document':
            m_mp_InitializeDocument(
              args.args[1] === undefined ? null : args.args[1],
            );
            break;
      ...
      default:
        if (func_name.includes('trigger')) {
          ...
          if (tmpArray.length > 2) {
            delete tmpArray[0];
            tmpArray = Enumerable.from(tmpArray)
              .where((x) => x !== undefined)
              .toArray()
              .join('.');
          } else tmpArray = tmpArray[1];
  ...
  if (parent === window) {
    ...
    function receiveMessage(event) {
      ...
      if (datas.exec === undefined) return;
      ...
      try {
        ...
        if (datas._integrated === true && datas.this !== false)
          ...
          if (datas.this !== false) eval(datas.exec);
        }
      } catch (error) {
        //console.error(error, datas);
      }
      //console.error("parent", datas, datas.child !== false);
      if (datas.child !== false) SynchroniseWorkspaces.PostToChilds(datas);
    ...
    function receiveMessage(event) {
      ...
      if (SynchroniseWorkspaces.im_who_have_lunched_this) {
        SynchroniseWorkspaces.im_who_have_lunched_this = false;
        if (event.data.eval !== 'always') return;
      }
