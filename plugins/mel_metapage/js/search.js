...
function mm_s_Action(e) {
  ...
  let search_icon = $('#barup-search-col .icofont-search')
    .removeClass('icofont-search')
    .html(
      '<span class="spinner-grow spinner-grow-sm" role="status"><span class="sr-only">Chargement des informations...</span></span>',
    );
  ...
  rcmail.env.mel_metapage_search = {
    ...
    timeout: function () {
      if (
        rcmail.env.mel_metapage_search.size ===
        rcmail.env.mel_metapage_search.finished.length
      ) {
        ...
        if (search_icon.children().hasClass('text-success')) {
          rcmail.display_message(
            'Recherche effectué avec succès !',
            'confirmation',
          );
        ...
        } else rcmail.display_message('Aucun résultat lié à la recherche...');
  ...
  rcmail.display_message(
    `Recherche de contenus lié au mot clé "${val}"`,
    'loading',
  );
  ...
  if (
    !Enumerable.from(jQuery._data($('body')[0], 'events')['click'])
      .where((x) => x.handler + '' === mm_s_bodyClick + '')
      .any()
  )
    //jQuery._data($("body")[0], 'events')["click"] === undefined)
...
function mm_s_bodyClick(event) {
  ...
  if (querry.hasClass('hidden')) return;
...
function mm_s_AfficheResults(datas, searchVal) {
  ...
  html =
    '<div class=search-parent><div class=search-label onclick="mm_s_extend(this)"><span class="icofont-rounded-down"></span><span class=result-label>' +
    datas.label +
    '</span></div><div>';
  ...
  for (let index = 0; index < datas.datas.length; index++) {
    ...
    html +=
      ...
      (element.onclick !== undefined
        ...
        : '') +
      ' class="result-link"><div class="result-header">' +
      element.header +
      '</div><div class="result-desc">' +
      element.sub_header +
      '</div></' +
      isa +
      '></td></tr>';
...
function mm_s_Calendar(cal) {
  ...
  window.location.href = rcmail.get_task_url(
    'calendar&source=' +
      cal.calendar +
      '&date=' +
      new Date(cal.start).getTime() / 1000.0,
  );
}
...
function mm_s_CreateOrUpdateFrame(action, args) {
  ...
  switch (action) {
    case 'mail':
      if ($('.mail-frame').length > 0) $('.mail-frame').remove();
      break;
    case 'contacts':
      if ($('.addressbook-frame').length > 0) $('.addressbook-frame').remove();
      break;
    default:
      break;
  }
