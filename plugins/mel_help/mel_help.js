  ...
  if (object.value.length > 3) {
    ...
    handle = setTimeout(function () {
      ...
      if (Object.keys(results).length) {
        ...
        document.getElementById('help-search-results').style =
          'display: block;';
        document.getElementById('help-search-results').innerHTML = '';
        ...
        for (const r of _res) {
          ...
          url.href =
            _search[r.key].action_url && !rcmail.env.ismobile
              ? _search[r.key].action_url
              : _search[r.key].help_url;
          url.target = '_blank';
          ...
          if (_search[r.key].help_url) {
            ...
            help_url.className =
              'hide-touch mel-button no-button-margin mel-before-remover mel-focus btn btn-secondary';
            help_url.innerHTML = _search[r.key].help_name
              ? "<span class='icon-mel-help mr-2'></span>" +
                _search[r.key].help_name
              : "<span class='icon-mel-help mr-2'></span>" +
                rcmail.get_label('help search open', 'mel_help');
          ...
          if (_search[r.key].action_url && !rcmail.env.ismobile) {
            ...
            action_url.className =
              'hide-touch bckg mel-button no-button-margin mel-before-remover mel-focus btn btn-secondary ml-4';
            action_url.innerHTML = _search[r.key].action_class
              ? "<span class='icon-mel-" +
                _search[r.key].action_class +
                " mr-2'></span>" +
                _search[r.key].action_name
              : "<span class='icon-mel-parameters-invert mr-2'></span>" +
                _search[r.key].action_name;
        ...
        document.getElementById('help-search-results').innerHTML =
          ...
          rcmail.get_label('help search no result', 'mel_help') +
          '</p>';
        document.getElementById('help-search-results').innerHTML += '<br/>';
        document.getElementById('help-search-results').innerHTML +=
          '<p>' + rcmail.get_label('help no result', 'mel_help') + '</p>';
        document.getElementById('help-search-results').innerHTML += '<br/>';
        ...
        document.getElementById('help-search-results').innerHTML +=
          "<button class='hide-touch mel-button bckg no-button-margin mel-before-remover mel-focus btn btn-secondary' onclick='rcmail.help_redirect()'>Ouvrir le salon de discussion<span class='icon-mel-unreads ml-3'></span></button>";
        $('.global-modal-body')
          .css('height', initial_modal_height)
          .css('overflow-y', 'auto')
          .css('overflow-x', 'hidden');
      }
    }, 300);
...
rcube_webmail.prototype.help_redirect = function () {
  help_popUp.close();
  rcmail.triggerEvent('help.redirect');
  ...
  if (object.value.length > 2) {
    ...
    handle = setTimeout(function () {
      ...
      if (Object.keys(results).length) {
        ...
        _res.sort((a, b) => (a.value < b.value ? 1 : -1));
        document.getElementById('video-search-results').style =
          'display: block;';
        document.getElementById('video-search-results').innerHTML = '';
        ...
        document.getElementById('video-search-results').innerHTML =
          "<p style='font-size:1.2rem'>" +
          rcmail.get_label('video search no result', 'mel_help') +
          '</p>';
