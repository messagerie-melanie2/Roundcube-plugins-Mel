 ...
 *
 * plugin mel_melanissimo pour roundcube
 *
...
class mel_melanissimo extends rcube_plugin {
  /**
   *
   * @var string
   */
  ...
  function init() {
    ...
    if ($this->rc->task == 'mail') {
      ...
      // Load stylesheet
      $this->include_stylesheet('melanissimo.css');

      // Requêtes ajax
      $this->register_action('plugin.test_melanissimo', array(
              $this,
              'testServiceMelanissimo'
      ));
      $this->register_action('plugin.send_melanissimo', array(
              $this,
      ...
      if ($this->rc->action == 'compose') {
        ...
        $this->include_script('melanissimo.js');
  ...
  /**
   * Fonction permettant de tester si le service Melanissmo doit être appelé au moment de l'envoie
   * Appelé en Ajax
   * Une autre fonction est appelé pour l'envoie par le service Melanissmo
   */
  public function testServiceMelanissimo() {
    if (mel_logs::is(mel_logs::DEBUG))
      mel_logs::get_instance()->log(mel_logs::DEBUG, "mel_melanissimo::testServiceMelanissimo()");
    ...
    $use_melanissimo = false;
    $over_size_melanissimo = false;
    $melanissimo_up = false;
    $size = 0;
    ...
    $result = array(
            'action' => 'plugin.test_melanissimo',
            'melanissimo_up' => $melanissimo_up,
            'use_melanissimo' => $use_melanissimo,
            'over_size_melanissimo' => $over_size_melanissimo,
            'size' => $size,
            'max_melanissimo_size' => $max_melanissimo_size,
            'dialog_html' => $this->_getHTMLDialogMelanissimo($COMPOSE_ID, $size),
            'unlock' => $unlock,
   ...
   * @return string $html
   */
  private function _getHTMLDialogMelanissimo($COMPOSE_ID, $size) {
    $COMPOSE = & $_SESSION['compose_data_' . $COMPOSE_ID];
