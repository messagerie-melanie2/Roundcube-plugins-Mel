...
function Nextcloud_Response(
  ...
  for (let index = 0; index < tmp.length; index++) {
    ...
    if (file.type !== mel_metapage.Symbols.nextcloud.folder || includeFolder) {
      if (file.is_valid_file !== false) this.files.push(file);
    }
  }
}
...
Nextcloud_Response.prototype.GetFile = function (filename) {
  var tmp = Enumerable.from(this.files)
    .where((x) => x.name.includes(filename))
    .firstOrDefault(null);
...
Nextcloud_Response.prototype.GetFiles = function (filename) {
  return Enumerable.from(this.files)
    .where((x) => x.name.includes(filename))
    .toArray();
};
