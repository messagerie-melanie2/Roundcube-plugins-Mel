...
function rcube_libcalendaring(settings) {
  ...
  if (!settings || !settings.date_format) return;
  ...
  this.event_date_text = function (event, voice) {
    ...
    var fromto,
      ...
      until = voice
        ? ' ' + rcmail.gettext('until', 'libcalendaring') + ' '
        : ' — ';
    ...
    if (event.allDay) {
      ...
      duration = end.getTime() / 1000 - start.getTime() / 1000;
      fromto =
        this.format_datetime(start, 1, voice) +
        (duration > 86400 || start.getDay() != end.getDay()
          ? until + this.format_datetime(end, 1, voice)
          : '');
    } else if (duration < 86400 && start.getDay() == end.getDay()) {
      fromto =
        this.format_datetime(start, 0, voice) +
  ...
  this.has_attendees = function (event) {
    return !!(
      ...
      (event.attendees.length > 1 ||
        String(event.attendees[0].email).toLowerCase() !=
  ...
  this.is_attendee = function (event, role, email) {
    ...
    if (
      !email &&
      !!this.calendars &&
      this.calendars[event.calendar] &&
      this.calendars[event.calendar]['owner_email']
    ) {
      ...
      if (settings.identity.emails.indexOf(';' + email) >= 0) {
        emails = settings.identity.emails;
      } else {
        emails = ';' + email;
      }
    } else {
      emails = email ? ';' + email.toLowerCase() : settings.identity.emails;
    }

    for (i = 0; event.attendees && i < event.attendees.length; i++) {
      if (
        (!role || event.attendees[i].role == role) &&
        event.attendees[i].email &&
        emails.indexOf(';' + event.attendees[i].email.toLowerCase()) >= 0
      ) {
  ...
  this.is_organizer = function (event, email) {
    // PAMELA - Mode assistantes
    if (event['organizer_calendar'] == event['calendar']) {
      return this.get_organizer(event);
    } else {
      return false;
    }
  ...
  this.parse_datetime = function (time, date) {
    ...
    var date = date
      ? $.datepicker.parseDate(
          this.datepicker_settings.dateFormat,
          date,
          this.datepicker_settings,
        )
      : new Date();
    ...
    if (!isNaN(time_arr[0])) {
      ...
      else if (time.match(/a[.m]*/i) && date.getHours() == 12) date.setHours(0);
    }
  ...
  this.date2ISO8601 = function (date) {
    ...
    if ('toDate' in date) return date.format('YYYY-MM-DD[T]HH:mm:ss'); // MomentJS
  ...
  this.format_datetime = function (date, mode, voice) {
    var res = '';
    if (!mode || mode == 1) {
      res += $.datepicker.formatDate(
        voice ? 'MM d yy' : this.datepicker_settings.dateFormat,
        date,
        this.datepicker_settings,
      );
    }
  ...
  this.text_color = function (color) {
    ...
    if (!color_map[color]) {
      ...
      if (/^#?([a-f0-9]{2})([a-f0-9]{2})([a-f0-9]{2})$/i.test(color)) {
        ...
        brightness =
          (parseInt(RegExp.$1, 16) * 299 +
            parseInt(RegExp.$2, 16) * 587 +
            parseInt(RegExp.$3, 16) * 114) /
          1000;
        if (brightness > 125) {
          color_map[color] = res;
        }
  ...
  this.text2html = function (str, maxlen, maxlines) {
    ...
    var link_pattern = new RegExp(
      '([hf]t+ps?://)(' + utf_domain + '([' + url1 + ']?[' + url2 + ']+)*)',
      'ig',
    );
    var mailto_pattern = new RegExp(
      '([^\\s\\n\\(\\);]+@' + utf_domain + ')',
      'ig',
    );
  ...
  this.init_alarms_edit = function (prefix, index) {
    ...
    edit_type.change(function () {
      $(this)
        .parent()
        .find('span.edit-alarm-values')
        [this.selectedIndex > 0 ? 'show' : 'hide']();
    });
    $(prefix + ' select.edit-alarm-offset').change(function () {
      ...
      parent
        .find('.edit-alarm-date, .edit-alarm-time')
        [val === '@' ? 'show' : 'hide']();
      parent
        .find('.edit-alarm-value')
        [val === '@' || val === '0' ? 'hide' : 'show']();
      parent.find('.edit-alarm-related')[val === '@' ? 'hide' : 'show']();
      parent
        .removeClass('offset-ontime offset-ondate offset-default')
        .addClass('offset-' + (class_map[val] || 'default'));
    });

    $(prefix + ' .edit-alarm-date')
      .removeClass('hasDatepicker')
      .removeAttr('id')
      .datepicker(this.datepicker_settings);
    ...
    $(prefix)
      ...
      .on('click', 'a.add-alarm', function (e) {
        var orig = $(this).closest('.edit-alarm-item'),
          ...
          item = orig.clone(false).removeClass('first').appendTo(orig.parent());
  ...
  this.set_alarms_edit = function (prefix, valarms) {
    ...
    for (i = 0; i < valarms.length; i++) {
      ...
      if (!alarm.action) alarm.action = 'DISPLAY';
      ...
      $('select.edit-alarm-related', domnode).val(
        /END/i.test(alarm.related) ? 'end' : 'start',
      );
      ...
      if (String(alarm.trigger).match(/@(\d+)/)) {
        ...
        $('input.edit-alarm-date', domnode).val(
          this.format_datetime(ondate, 1),
        );
        $('input.edit-alarm-time', domnode).val(
          this.format_datetime(ondate, 2),
        );
      ...
      if (alarm.action) {
        ...
        if (offset == '@') {
          alarm.trigger =
            ...
            me.date2unixtime(
              me.parse_datetime(
                $('input.edit-alarm-time', elem).val(),
                $('input.edit-alarm-date', elem).val(),
              ),
            );
        } else if (offset === '0') {
          alarm.trigger = '0S';
        } else if (
          !isNaN((val = parseInt($('input.edit-alarm-value', elem).val()))) &&
          val >= 0
        ) {
  ...
  var time_autocomplete_list = function (p, callback) {
    ...
    var st,
      ...
      start =
        ...
        $('#' + m[1] + '-startdate').val() == $('#' + m[1] + '-enddate').val()
          ? me.parse_datetime(st, '')
          : null,
      full = p.term - 1 > 0 || p.term.length > 1,
      hours = start
        ? start.getHours()
        : (full ? me.parse_datetime(p.term, '') : now).getHours(),
    ...
    while (h < 24) result.push(time_autocomplete_format(h++, 0, start));
  ...
  var time_autocomplete_open = function (event, ui) {
    ...
    if (val === '') menu._scrollIntoView(widget.children('li:first'));
    else
      widget.children().each(function () {
        ...
        if (html.indexOf(val) == 0) menu._scrollIntoView(li);
      });
  };
  ...
  this.display_alarms = function (alarms) {
    ...
    if (this.alarm_dialog) this.alarm_dialog.dialog('destroy').remove();
    ...
    for (i = 0; i < alarms.length; i++) {
      ...
      if (
        alarm.location.includes('@visio') ||
        alarm.location.includes('#visio') ||
        alarm.location.includes(rcmail.env['webconf.base_url'])
      ) {
        let link;
        if (alarm.location.includes('@visio')) {
          ...
          const categoryExist =
            alarm.categories !== undefined &&
            alarm.categories !== null &&
            alarm.categories.length > 0;
          const isWsp = categoryExist && alarm.categories[0].includes('ws#');
          const ariane = isWsp ? 'null' : "'@home'";
          const wsp = isWsp
            ? `'${alarm.categories[0].replace('ws#', '')}'`
            : 'null';
          link = `href="#" onclick="window.webconf_helper.go('${mel_metapage.Functions.webconf_url(alarm.location)}', ${wsp}, ${ariane})"`;
        }
        html +=
          '<div class="event-section"><a title="' +
          rcmail.gettext('openwebconf_title', 'libcalendaring') +
          '" ' +
          link +
          '>' +
          rcmail.gettext('openwebconf', 'libcalendaring') +
          '</a></div>';
      } else {
        html +=
          '<div class="event-section">' + Q(alarm.location || '') + '</div>';
      }
      ...
      actions = $('<div>')
        .addClass('alarm-actions')
        .append(adismiss.data('id', alarm.id))
        .append(asnooze.data('id', alarm.id));
      records.push(
        $('<div>').addClass('alarm-item').html(html).append(actions),
      );
    ...
    if (audio_alarms.length) this.audio_alarms(audio_alarms);
  ...
  this.audio_alarms = function (alarms) {
    ...
    rcmail.display_message(
      rcmail.gettext('alarmtitle', 'libcalendaring') + ': ' + Q(txt.join(', ')),
      'notice',
      10000,
    );
  ...
  this.snooze_dropdown = function (link, event) {
    if (!this.snooze_popup) {
      ...
      if (!this.snooze_popup.length) {
        this.snooze_popup = $('<div>')
          .attr('id', 'alarm-snooze-dropdown')
          .addClass('popupmenu')
  ...
  this.dismiss_alarm = function (id, snooze, event) {
    ...
    rcmail
      .http_post('utils/plugin.alarms', {
        action: 'dismiss',
        data: { id: id, snooze: snooze },
      })
      // PAMELA - Correction popup infini
      .always(() => {
        rcmail.triggerEvent('calendar.dismiss.after', {
          id,
          snooze,
          _event: event,
        });
    ...
    if (this.dismiss_link) {
      ...
      var new_ids = jQuery.grep(this.alarm_ids, function (v) {
        return v != id;
      });
      if (new_ids.length) this.alarm_ids = new_ids;
      else this.alarm_dialog.dialog('close');
    }
  ...
  this.init_recurrence_edit = function (prefix) {
    ...
    $('#edit-recurrence-frequency').change(function (e) {
      ...
      if (freq) {
        ...
        if (freq != 'rdate') $('#recurrence-form-until').show();
      }
    });
    $('#recurrence-form-rdate input.button.add').click(function (e) {
      var dt,
        dv = $('#edit-recurrence-rdate-input').val();
    ...
    $('#edit-recurrence-repeat-times').change(function (e) {
      $('#edit-recurrence-repeat-count').prop('checked', true);
    });
  ...
  this.set_recurrence_edit = function (rec) {
    var date,
      recurrence = $('#edit-recurrence-frequency')
        .val(
          rec.recurrence
            ? rec.recurrence.FREQ || (rec.recurrence.RDATE ? 'RDATE' : '')
            : '',
        )
        .change(),
      interval = $('.recurrence-form select.edit-recurrence-interval').val(
        rec.recurrence ? rec.recurrence.INTERVAL || 1 : 1,
      ),
      rrtimes = $('#edit-recurrence-repeat-times').val(
        rec.recurrence ? rec.recurrence.COUNT || 1 : 1,
      ),
      rrenddate = $('#edit-recurrence-enddate').val(
        rec.recurrence && rec.recurrence.UNTIL
          ? this.format_datetime(this.parseISO8601(rec.recurrence.UNTIL), 1)
          : '',
      );
    $('.recurrence-form input.edit-recurrence-until:checked').prop(
      'checked',
      false,
    );
    ...
    if (rec.recurrence && rec.recurrence.COUNT)
      rrepeat_id = '#edit-recurrence-repeat-count';
    else if (rec.recurrence && rec.recurrence.UNTIL)
      rrepeat_id = '#edit-recurrence-repeat-until';
    $(rrepeat_id).prop('checked', true);

    if (
      rec.recurrence &&
      rec.recurrence.BYDAY &&
      rec.recurrence.FREQ == 'WEEKLY'
    ) {
    ...
    if (rec.recurrence && rec.recurrence.BYMONTHDAY) {
      $('input.edit-recurrence-monthly-bymonthday').val(
        String(rec.recurrence.BYMONTHDAY).split(','),
      );
    ...
    if (
      rec.recurrence &&
      rec.recurrence.BYDAY &&
      (rec.recurrence.FREQ == 'MONTHLY' || rec.recurrence.FREQ == 'YEARLY')
    ) {
      var byday,
        section = rec.recurrence.FREQ.toLowerCase();
    ...
    if (rec.recurrence && rec.recurrence.BYMONTH) {
      $('input.edit-recurrence-yearly-bymonth').val(
        String(rec.recurrence.BYMONTH).split(','),
      );
    } else if (rec.start) {
      date = 'toDate' in rec.start ? rec.start.toDate() : rec.start;
      $('input.edit-recurrence-yearly-bymonth').val([
  ...
  this.serialize_recurrence = function (timestr) {
    ...
    if (freq != '') {
      ...
      if (until == 'count')
        ...
        recurrence.UNTIL = me.date2ISO8601(
          me.parse_datetime(
            timestr || '00:00',
            $('#edit-recurrence-enddate').val(),
          ),
        );

      if (freq == 'WEEKLY') {
        var byday = [];
        $('input.edit-recurrence-weekly-byday:checked').each(function () {
          byday.push(this.value);
        });
        if (byday.length) recurrence.BYDAY = byday.join(',');
      } else if (freq == 'MONTHLY') {
        var mode = $('input.edit-recurrence-monthly-mode:checked').val(),
          bymonday = [];
        if (mode == 'BYMONTHDAY') {
          $('input.edit-recurrence-monthly-bymonthday:checked').each(
            function () {
              bymonday.push(this.value);
            },
          );
          if (bymonday.length) recurrence.BYMONTHDAY = bymonday.join(',');
        } else
          recurrence.BYDAY =
            $('#edit-recurrence-monthly-prefix').val() +
            $('#edit-recurrence-monthly-byday').val();
      } else if (freq == 'YEARLY') {
        var byday,
          bymonth = [];
        $('input.edit-recurrence-yearly-bymonth:checked').each(function () {
          bymonth.push(this.value);
        });
        if (bymonth.length) recurrence.BYMONTH = bymonth.join(',');
  ...
  this.add_rdate = function (date) {
    ...
    $('<a>')
      .attr({
        href: '#del',
        class: 'iconbutton delete icon button',
        title: rcmail.get_label('delete', 'libcalendaring'),
      })
      .append(
        $('<span class="inner">').text(
          rcmail.get_label('delete', 'libcalendaring'),
        ),
      )
      .appendTo(li);
  };
  ...
  this.expand_attendee_group = function (e, add, remove) {
    var id = (e.data ? e.data.email : null) || $(e.target).attr('data-email'),
      role_select = $(e.target)
        .closest('tr')
        .find('select.edit-attendee-role option:selected');

    this.group2expand[id] = {
      link: e.target,
      data: $.extend({}, e.data || {}),
      adder: add,
      remover: remove,
  ...
  this.expand_attendee_callback = function (result) {
    ...
    if (data && data.adder && result.members && result.members.length) {
      ...
      rcmail.display_message(
        result.error ||
          rcmail.gettext('expandattendeegroupnodata', 'libcalendaring'),
        'error',
      );
    }
  };
  ...
  this.render_message_links = function (links, container, edit, plugin) {
    ...
    $.each(links, function (i, link) {
      if (!link.mailurl) return true; // continue
      ...
      if (edit) {
        $('<a>')
          .attr({
            href: '#delete',
            title: rcmail.gettext('removelink', plugin),
            'data-uri': link.uri,
            class: 'delete',
          })
...
rcube_libcalendaring.attendee_html = function (data) {
  ...
  if (data['delegated-to'])
    tooltip =
      rcmail.gettext('libcalendaring.delegatedto') + ' ' + data['delegated-to'];
  ...
  return $('<span>')
    .append(
      $('<span>')
        .attr({ class: 'attendee ' + status, title: tooltip })
        .append(name.text(dispname)),
    )
    .html();
};
 ...
 *
 */
rcube_libcalendaring.add_from_itip_mail = function (
  mime_id,
  task,
  status,
  dom_id,
  ...
  rcmail.env.itip_current_mail_selected = rcmail.message_list
  ...
  if (status == 'delegated') {
    rcube_libcalendaring.itip_delegate_dialog(
      function (data) {
        rcmail.http_post(
          task + '/itip-delegate',
          {
  ...
  if (dom_id) {
    ...
    if (!noreply) comment = $('#reply-comment-' + dom_id).val();
  }
...
rcube_libcalendaring.itip_delegate_dialog = function (callback, selector) {
  // PAMELLA
  rcmail.triggerEvent('rcube_libcalendaring.itip_delegate_dialog.before', {
    callback,
    selector,
  });
  ...
  if (selector && selector.length) {
    form.append(
      $('<div class="form-section form-group">')
        .append(
          $('<label for="itip-saveto">').text(
            rcmail.gettext('libcalendaring.savein'),
          ),
        )
  ...
  buttons.push({
    ...
    click: function () {
      var doc = window.parent.document,
        delegatee = String($('#itip-delegate-to', doc).val()).replace(
          /(^\s+)|(\s+$)/,
          '',
        );

      //PAMELA
      if (delegatee === String(undefined))
        delegatee = String($('#itip-delegate-to').val()).replace(
          /(^\s+)|(\s+$)/,
          '',
        );
  ...
  buttons.push({
    ...
    click: function () {
      // PAMELLA
      rcmail.triggerEvent('rcube_libcalendaring.itip_delegate_dialog.close', {
        item: this,
      });
      dialog.dialog('close');
    },
  ...
  dialog = rcmail.show_popup_dialog(
    form,
    rcmail.gettext('delegateinvitation', 'itip'),
    buttons,
    {
      width: 460,
      open: function (event, ui) {
        ...
        rcm.init_address_input_events(
          $(this).find('#itip-delegate-to').focus(),
          ac_props,
        );
        rcm.env.recipients_delimiter = '';
        // PAMELLA
        rcmail.triggerEvent('rcube_libcalendaring.itip_delegate_dialog.open', {
          item: this,
...
rcube_libcalendaring.itip_rsvp_recurring = function (btn, callback, event) {
  ...
  if (!menu.length) {
    menu = $('<div>')
      .attr({ class: 'popupmenu', id: 'itip-rsvp-menu', 'aria-hidden': 'true' })
      .appendTo(document.body);
    list = $('<ul>')
      .attr({ class: 'toolbarmenu menu', role: 'menu' })
      .appendTo(menu);

    $.each(['all', 'current' /*,'future'*/], function (i, mode) {
      var link = $('<a>')
        .attr({ class: 'active', rel: mode })
        .text(rcmail.get_label('rsvpmode' + mode))
        .on('click', function () {
          //PAMELA
          rcmail.hide_menu('itip-rsvp-menu', event);
          callback(
            /* PAMELA */ rcube_libcalendaring.itip_rsvp_recurring.action ===
              undefined
              ? action
              : rcube_libcalendaring.itip_rsvp_recurring.action,
            $(this).attr('rel'),
          );
        });
...
rcube_libcalendaring.remove_from_itip = function (event, task, title) {
  rcmail.confirm_dialog(
    rcmail.gettext('itip.deleteobjectconfirm').replace('$title', title),
    'delete',
    function () {
      rcmail.http_post(
        task + '/itip-remove',
...
rcube_libcalendaring.decline_attendee_reply = function (mime_id, task) {
  ...
  var html =
    '<div class="itip-dialog-confirm-text">' +
  ...
  dialog = rcmail.show_popup_dialog(
    html,
    rcmail.gettext('declineattendee', 'itip'),
    buttons,
    {
      width: 460,
      open: function () {
...
rcube_libcalendaring.refetch_itip_object_status = function (p, resp, calid) {
  ...
  $('.calendar-agenda-preview .event-row-before').not(':first').remove();
  $('.calendar-agenda-preview .event-row-before').replaceWith('%before%');
  $('.calendar-agenda-preview .event-row-after').not(':first').remove();
...
rcube_libcalendaring.remove_label_from_itip_mail = function () {
  ...
  if (typeof rc.mel_label_toggle === 'function') {
    ...
    if (
      ...
      jQuery.inArray(label, tb_labels_for_message) !== -1
    ) {
...
rcube_libcalendaring.update_itip_object_status = function (p) {
  ...
  if (
    tb_labels_for_message &&
    jQuery.inArray('_-t-_rdvtraite', tb_labels_for_message) !== -1
  ) {
    $('#label-' + p.id).show();
    return;
  }
  ...
  if (p.action == 'rsvp') {
    $('#rsvp-' + p.id + ' input.button')
      .prop('disabled', false)
      .filter('.' + String(p.status || 'unknown').toLowerCase())
      .prop('disabled', p.latest);
  ...
  if (p.rescheduled) $('.calendar-eventdetails td.date').addClass('modified');
  ...
  if (window.UI && UI.pretty_select) {
    $('#rsvp-' + p.id + ' select').each(function () {
      UI.pretty_select(this);
    });
  }
};
...
rcube_libcalendaring.open_itip_preview = function (url, msgref) {
  if (!rcmail.env.itip_existing) url += '&itip=' + escape(msgref);
  ...
  rcmail.addEventListener('init', function (evt) {
    ...
    rcmail
      .addEventListener(
        ...
        rcube_libcalendaring.update_itip_object_status,
      )
      .addEventListener(
        'plugin.fetch_itip_object_status',
        rcube_libcalendaring.fetch_itip_object_status,
      )
      .addEventListener(
        'plugin.itip_message_processed',
        rcube_libcalendaring.itip_message_processed,
      );
  });
