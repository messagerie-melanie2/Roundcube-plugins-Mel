class SearchAction {
  ...
  html(classes = '') {
    if (this.text === '') return '';
    return $(
      `<button class="$${classes} mel-button btn btn-secondary">${this.text}<span class="plus icon-mel-arrow-right"></span></button>`,
    ).click(this.onclick);
  }
}
...
SearchResultCalendar.from_array = async function (cals) {
  ...
  for (let index = 0; index < cals.length; ++index) {
    if (
      SearchResult.calendarToReadable(cals[index].calendar) !==
      rcmail.env.username
    )
      continue;
  ...
  await Promise.allSettled(
    Enumerable.from(retour)
      .select((x) => x.waiting_loaded())
      .toArray(),
  );
...
SearchResult.parse = function (string) {
  ...
  const ap = 'µ¤¤µ';
  const n = '-¤¤n¤¤-';
  if (string.includes(key)) string = string.replaceAll(key, '"');

  if (string.includes(ap)) string = string.replaceAll(ap, "'");
  ...
  if (parent === window && rcmail.env.task === 'calendar') {
    ...
    if (querry.length > 0) {
      ...
      new Promise(async (a, b) => {
        while (rcmail.env.frame_created === false) {
          await delay(100);
          if (!rcmail.busy) rcmail.set_busy(true, 'loading');
        }
        rcmail.set_busy(false);
        rcmail.clear_messages();
        $('#barup-search-background').addClass('hidden');
