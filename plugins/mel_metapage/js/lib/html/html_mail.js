...
export class mail_html extends mel_html2 {
  ...
  _generate_date() {
    ...
    if (
      moment(base_date).startOf('day').format() ===
      moment(now).startOf('day').format()
    )
      date = new mel_html(
        'span',
        { class: base_class },
        moment(base_date).format('HH:mm'),
      );
    else {
      ...
      if (ellapsed_days < 7) {
        date.locale('fr');
        left_date = mel_html.div(
          { class: `${base_class}-left` },
          rcube_calendar.mel_metapage_misc
            .GetDateFr(date.format('dddd'))
            .slice(0, 3) + ' -',
        );
      } else if (
        moment(base_date).startOf('year').format() !==
        moment(now).startOf('year').format()
      )
        left_date = mel_html.div(
          { class: `${base_class}-left` },
      ...
      const right_date = mel_html.div(
        { class: `${base_class}-right` },
        date.format('HH:mm'),
      );

      date = new mel_html2('div', {
        attribs: { class: base_class },
        contents: [left_date, right_date],
      });
  ...
  async _action_on_click() {
    ...
    if (
      !have_frame ||
      (have_frame &&
        FramesManager.Instance.get_frame('mail')[0].contentWindow.rcmail.env
          .mailbox !== 'INBOX')
    ) {
      have_frame = false;
      config['_mbox'] = 'INBOX';
