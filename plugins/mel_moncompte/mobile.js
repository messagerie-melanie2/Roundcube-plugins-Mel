...
function handleResyncAction(e) {
  // Gérer la navigation au clic ou au clavier
...
$(document).on(
  {
    click: handleDeviceAction,
    keydown: handleDeviceAction,
  },
  '#mobile_actions a',
);
...
if (window.rcmail) {
  rcmail.addEventListener(
    'responseafterplugin.statistics.zpush_command',
    function (evt) {
      rcmail.display_message(evt.response.message, evt.response.status);
      if (evt.response.status == 'success' && evt.response.command != 'Wipe') {
  ...
  rcmail.addEventListener('init', function (evt) {
    ...
    if (rcmail.gui_objects.mel_statistics_mobiles_list) {
      rcmail.mobiles_list = new rcube_list_widget(
        rcmail.gui_objects.mel_statistics_mobiles_list,
        { multiselect: false, draggable: false, keyboard: true },
      );
      rcmail.mobiles_list.addEventListener('select', function (e) {
        p.mel_statistics_mobile_select(e);
      });
...
rcube_webmail.prototype.load_mobile_frame = function (id) {
  ...
  if (
    ...
    target.location.href =
      this.env.comm_path +
      '&_action=plugin.mel_statistics_mobile&_framed=1' +
      (has_id ? '&_fid=' + id : '') +
      '&_unlock=' +
      msgid;
  }
