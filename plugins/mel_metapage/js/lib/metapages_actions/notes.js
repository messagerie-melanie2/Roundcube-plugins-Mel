...
export class MetapageNotesModule extends MetapageModule {
  ...
  main() {
    ...
    this.select_note_button().click(() => {
      if (!this._fullscreen) {
        ...
        this._create_fullscreen_item()._generate_notes().show();
      } else this.toggle();
    });
    ...
    this.add_event_listener(
      ...
      async (taked) => {
        ...
        if (taked.pin) {
          ...
          if (
            $(`#note-pin-${taked.uid.replace('pin-', '').replace('note-', '')}`)
              .length === 0
          ) {
            const init_pos =
              rcmail.env.mel_metapages_notes[
                taked.uid.replace('pin-', '').replace('note-', '')
              ].pin_pos;
            const has_pos = !!init_pos?.[0] && !!init_pos?.[1];
            const x =
              init_pos?.[0] ?? Random.intRange(75, window.innerWidth - 315);
            const y =
              init_pos?.[1] ?? Random.intRange(60, window.innerHeight / 4);
            ...
            if (!has_pos) {
              ...
              rcmail.env.mel_metapages_notes[
                taked.uid.replace('pin-', '').replace('note-', '')
              ].pin_pos_init = [window.outerWidth, pos.y];
              await taked.post(
                'pin_move',
                {
  ...
  _on_resize() {
    $('.mel-note.pined').each((i, e) => {
      ...
      const note =
        rcmail.env.mel_metapages_notes[
          $(e).attr('id').replace('pin-', '').replace('note-', '')
        ];
  ...
  _create_fullscreen_item() {
    ...
    this._fullscreen.$apps[0].addEventListener('dragover', (ev) => {
      ...
      if (!$(ev.target).hasClass('fullscreen_container')) {
        ...
        if ($parent.hasClass('mel-note')) {
          ...
          if (x >= $parent.width() / 2)
            $parent.addClass('dragover-right').removeClass('dragover-left');
    ...
    this._fullscreen.$apps[0].addEventListener('drop', (ev) => {
      ...
      if ($target.length > 0) {
        let sticker_target = Sticker.fromHtml(
          $target.attr('id').replace('note-', EMPTY_STRING),
        );
  ...
  _generate_notes() {
    ...
    let stickers = [];
    let current_sticker;
    for (const iterator of MelEnumerable.from(this.notes)
      .where((x) => !!x.value.uid)
  ...
  _generate_plus_button() {
    ...
    const text = new mel_html(
      'span',
      { class: 'new-note-text' },
      this.gettext('new-note', 'mel_metapage'),
    );
    let html_button = new mel_html2('button', {
      attribs: {
        class: `new-note-button ${MaterialSymbolHtml.get_class_fill_on_hover()}`,
        title: this.gettext('note-create-title', 'mel_metapage'),
      },
      contents: [icon, text],
    });
  ...
  _generate_pined_notes() {
    ...
    let current_sticker;
    for (const iterator of MelEnumerable.from(this.notes).where(
      (x) => x.value.pin === true || x.value.pin === 'true',
    )) {
      const { key, value: note } = iterator;
      current_sticker = Sticker.from(note);
      ...
      current_sticker
        .generate({
          pos: new Point(
            parseInt(note.pin_pos?.[0] ?? 60),
            parseInt(note.pin_pos?.[1] ?? 60),
          ),
        })
        .appendTo($('body'));
