...
JsHtml.extend('card_container', function(attribs = {}) {
    ...
    if (!!attribs['data-card-img-top']) {
        card = card.card_img_top({src: attribs['data-card-img-top'], alt: attribs['data-card-img-top-alt'] || attribs['data-card-img-top']});
    }
    ...
    if (!!attribs['data-card-footer-link-max']) {
        ...
        for (let index = 1; index <= max; index++) {
            card = card.card_link({href: attribs[`data-card-footer-link-${index}`]}).text(attribs[`data-card-footer-link-${index}-text`]).end();
        }
    }
