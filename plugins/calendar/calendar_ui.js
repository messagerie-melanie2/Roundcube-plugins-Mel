...
function rcube_calendar_ui(settings)
{
    ...
    var event_show_dialog = function(event, ev, temp)
    {
      ...
      $dialog.find('div.event-section, span.event-section, div.event-line, span.event-line').hide();
      ...
      if (event.description) {
    	  ...
    	  $('#event-description-attachments').show()
      }
      if (event.vurl)
      ...
      if (calendar.name)
        $('#event-calendar').show().children('.event-text').html(Q(calendar.name)).attr('class', 'event-text cal-'+calendar.id);
      if (event.categories)
      ...
      me.dialog_resize($dialog.get(0), $dialog.height(), 520);
    ...
    var event_edit_dialog = function(action, event)
    {
      ...
      // PAMELA
      var startdate = $('#edit-startallday').val($.fullCalendar.formatDate(event.start, settings['date_format'])).data('duration', duration);
      var enddate = $('#edit-endallday').val($.fullCalendar.formatDate(event.end, settings['date_format']));
      var period = $('#edit-period').get(0);
...
//        endtime.val("13:00").hide();
        ...
        $('.period-cell').addClass('disabled');
        $('.all-day-cell').removeClass('disabled');
      }
      else {
        allday.checked = false;
        // PAMELA
        period.checked = true;
        $('.period-cell').removeClass('disabled');
        $('.all-day-cell').addClass('disabled');
      }
      ...
      me.dialog_resize($dialog.get(0), editform.height() + (bw.ie ? 20 : 0), 650);
    ...
    var event_freebusy_dialog = function()
    {
      ...
      buttons[rcmail.gettext('select', 'calendar')] = function() {
        ...
        // PAMELA
        $('#edit-startallday').val(freebusy_ui.startdate.val());
        $('#edit-endallday').val(freebusy_ui.enddate.val());
    ...
    var event_times_changed = function()
    {
      if (me.selected_event) {
        ...
        // PAMELA
        me.selected_event.start = parse_datetime(allday.checked ? '12:00' : $('#edit-starttime').val(), allday.checked ? $('#edit-startallday').val() : $('#edit-startdate').val());
        me.selected_event.end   = parse_datetime(allday.checked ? '13:00' : $('#edit-endtime').val(), allday.checked ? $('#edit-endallday').val() : $('#edit-enddate').val());
        if (allday.checked) {
          $('#edit-startdate').val($('#edit-startallday').val());
          $('#edit-enddate').val($('#edit-endallday').val());
        }
        else {
          $('#edit-startallday').val($('#edit-startdate').val());
          $('#edit-endallday').val($('#edit-enddate').val());
        }
        if (event_attendees)
          freebusy_ui.needsupdate = true;
    ...
    calendars_list.addEventListener('select', function(node) {
      if (node && node.id && me.calendars[node.id]) {
        ...
        // MANTIS 0005957: Roue crantée pour le calendrier
    ...
    var shift_enddate = function(dateText) {
      ...
      // PAMELA
      $('#edit-endallday').val($.fullCalendar.formatDate(newend, me.settings['date_format']));
      event_times_changed();
    };
    ...
    var init_calendar_ui = function()
    {
      ...
      // PAMELA
      $('#edit-endallday').datepicker(datepicker_settings);
      $('#edit-startallday').datepicker(datepicker_settings).datepicker('option', 'onSelect', shift_enddate).change(function(){ shift_enddate(this.value); });
      $('#edit-endallday').datepicker('option', 'onSelect', event_times_changed).change(event_times_changed);
