...
export class SensitivityPart extends FakePart {
  ...
  constructor($select, $button, $icon, $dialog) {
     ...
     */
    this._title =
      this._$dialog.dialog('option', 'title') || 'Créer un évènement';
  ...
  onUpdate(val) {
    this._$icon
      .html(SensitivityPart.ICONS[val] ?? '')
      .parent()
  ...
  onClick(...args) {
    ...
    if ($e.attr('aria-valuenow') === SensitivityPart.STATES.private)
      $e.attr('aria-valuenow', SensitivityPart.STATES.public);
