...
/*                 AUDIOS MANAGER
/*
/* Contient plusieurs classes qui permet de visualiser les entrées micros ou de tester les sortis audio
/*****************************************************************/
//¤Constantes
/**
 * Nombre de "barre" de visualisation
 */
const visualValueCount = 16;
/**
 * Url de l'audio à qui sera joué
 */
const audiourltotest = mel_metapage.Functions.url('', '').split('?')[0] + 'plugins/mel_metapage/skins/elastic/audio/test.mp3';
   ...
   */
    ...
    async start(devices = null) {
      ...
      for (const iterator of devices) {
        if (iterator.kind === this.linkedDevice.kind && iterator.label === this.linkedDevice.label) {
...
class MelAudioStruct{
   ...
   */
     ...
     * @private Cette fonction est privé, ne l'utilisez pas en dehord de cette classe !
     * @returns Chaîne
     */
    _init() {
      /**
       ...
       */
        ...
        this.$main = null;
        /**
         * @type {Array<$>} Les "barres" de visualisations
         */
        this.elements = [];
        /**
...
class MelAudioManager {
    ...
    async start($main, alreadyExistingDevices = null) {
        const devices = alreadyExistingDevices || await navigator.mediaDevices.enumerateDevices();
        this.devices = devices;

        for (const dvc of devices) {
            if (dvc.kind === 'audioinput')
            {
                this.visualElements[dvc.deviceId] = (new MelAudioStruct(dvc, $('<div class="rotovisua"><div class="text"></div><div class="visu"></div></div>').data('deviceid', dvc.deviceId).appendTo($main).find('.visu'))).generateElements();
                this.visualElements[dvc.deviceId].visualizer = new AudioVisualizer(dvc, this.visualElements[dvc.deviceId], new AudioContext() ,MelAudioManager.processFrame, MelAudioManager.processError);
                this.visualElements[dvc.deviceId].$main.parent().find('.text').html(dvc.label);
              }
        }
    ...
    async addElement(dvc, $button){
      ...
      if (dvc.kind === 'audioinput')
      {
          this.visualElements[dvc.deviceId] = (new MelAudioStruct(dvc, $('<div class="rotovisua"><div class="button"></div><div class="visu"></div></div>').data('deviceid', dvc.deviceId).find('.visu'))).generateElements();
          this.visualElements[dvc.deviceId].visualizer = await (new AudioVisualizer(dvc, this.visualElements[dvc.deviceId], new AudioContext() ,MelAudioManager.processFrame, MelAudioManager.processError)).start(this.devices);
          this.visualElements[dvc.deviceId].$main.parent().find('.button').html($button);
      }
...
//Pour tester sur la page de test
// var melaudio = new MelAudioManager();
// $(document).ready(() => {

//   $('#tstart').click(async () => {
//       //debugger;
//       await melaudio.start($('#canvas-visualizer'));

//       for (const key in melaudio.visualElements) {
//         if (Object.hasOwnProperty.call(melaudio.visualElements, key)) {
//           melaudio.visualElements[key].visualizer.start(melaudio.devices);
//         }
//       }

//       for (const dvc of melaudio.devices) {
//         if (dvc.kind === 'audiooutput')
//         {
//           $('#canvas-visualizer').append($(`<button data-idlabel="${dvc.deviceId}">${dvc.label}</button>`).click(e => {
//             e = $(e.currentTarget);
//             MelAudioTester.test({deviceId:e.data('idlabel')});
//           }))
//         }
//       }
//   });
