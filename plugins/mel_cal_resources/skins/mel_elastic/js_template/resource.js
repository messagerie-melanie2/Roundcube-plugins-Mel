        ...
        .div({ class: 'rc-page-params' })
            .row()
                .col_6().css('flex-wrap', 'unwrap').css('display', 'flex')
                    .div()
                        .icon('schedule').end()
                    .end()
                    .input_text({ class:'input-date-start', datepicker: true, value:date.format(DATE_FORMAT), onchange:resource._functions.on_date_start_changed })
                    .input_time({ class:'input-time-start', value:date.format(DATE_HOUR_FORMAT), onchange:resource._functions.on_time_start_changed }).css('display', resource.all_day ? 'none' : EMPTY_STRING)
                .end()
                .col_6()
            ...
            .row()
                .col_6().css('flex-wrap', 'unwrap').css('display', 'flex')
                    ...
                    .end()
                    .input_text({ class:'input-date-end', datepicker: true, value:end_date.format(DATE_FORMAT), onchange:resource._functions.on_date_end_changed })
                    .input_time({ class:'input-time-end', value:end_date.format(DATE_HOUR_FORMAT), onchange:resource._functions.on_time_end_changed }).css('display', resource.all_day ? 'none' : EMPTY_STRING)
                .end()
                .col_6()
                .end()
            .end()
