...
export class FolderIcon extends AFolderModifier {
      ...
      (args) => {
        ...
        popup.on_create_default_items.push(() => {
          return MelHtml.start
            ...
            .a({
              class: `${icon ? '' : 'selected-item'} mel-button no-button-margin no-margin-button`,
            })
            .css({
              'border-radius': '5px',
              'margin-right': '5px',
            ...
            .attr(
              'onmouseenter',
              this._default_button_on_hover.bind(this, selector),
            )
            .attr(
              'onmouseleave',
              this._default_button_on_leave.bind(this, selector),
            )
        ...
        if (!this.get_skin().css_rules.ruleExist(icon_key)) {
          this.get_skin().css_rules.addAdvanced(
            icon_key,
            `#${popup.list_container_id}.folderlist li a::before`, //bnum-folder-icons-container
            'margin: 0 !important;',
        ...
        popup.on_save.push(async (popup, dialog) => {
          ...
          const result = await BnumConnector.connect(
            BnumConnector.connectors.mail_set_folder_icon,
            { params: config },
          );
        ...
        popup.on_cancel.push(
          popup.on_close.events[func_id].callback.bind(this),
        );
  ...
  _default_button_on_hover(selector, e) {
    ...
    let $previsu = this._save_default_icon().html(
      MelHtml.start.icon('default').end().generate(),
    );
  ...
  _default_click(selector, e) {
    ...
    e = $(e.currentTarget);
    $('#bnum-folder-icons-container .selected-item').removeClass(
      'selected-item',
    );
  ...
  update_visuel() {
    ...
    for (const key in folders) {
      if (Object.hasOwnProperty.call(folders, key)) {
        ...
        // $(`#mailboxlist a[rel="${key}"]`)
        //   .parent()
        //   .addClass(`bnum-updated-${icon}`);

        // $(`#favorite-folders li[mailid="${this._get_true_key(key)}"]`).addClass(
        //   `bnum-updated-${icon}`,
        // );
  ...
  async get_from_server() {
    await BnumConnector.connect(
      BnumConnector.connectors.mail_get_folders_icon,
      {},
    );
  }
}
