...
class ModuleMail extends BaseModule {
  ...
  start() {
    ...
    this.show_last_mails({}).then(() => {
      loaded = true;
      this._set_listeners().trigger_event(
        'portal.mails.after',
        { module: this },
        { top: true },
      );
  ...
  _init() {
    ...
    mel_html2
      .div({
        attribs: { class: 'melv2-email-undefined-content' },
        contents: loader,
      })
      .create($contents);

    let ul = new html_ul({
      attribs: { class: 'melv2-email-ul ignore-bullet' },
    });

    for (let index = 0, html_element; index < ModuleMail.MAX; ++index) {
      html_element = new mail_html(null)
        .setId(`melv2-email-id-${index}`)
        .css('display', 'none');
...
class MailLoaderStockage extends MailLoaderBase {
  ...
  need_refresh() {
    ...
    const LAST_UPDATE_DATE = this.load(
      LAST_UPDATE_KEY,
      LAST_UPDATE_DATE_DEFAULT_VALUE,
    );
    const IS_NEXT_DAY =
      !LAST_UPDATE_DATE ||
      moment(LAST_UPDATE_DATE).startOf('day') !== moment().startOf('day');
...
class MailLoader extends MailLoaderBase {
  ...
  start() {
    super.start();
    this.loaders = [];
    this /*.addLoader(new MailLoaderStockage())*/.addLoader(
      new MailLoaderDataBase(),
    );
  }
