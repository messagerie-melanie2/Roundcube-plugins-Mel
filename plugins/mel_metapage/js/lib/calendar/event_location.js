...
class CustomVisioLocation extends ActionLocation {
	constructor(location) {
		if (CustomVisioLocation.is_custom_visio(location))
			location = location.replace(CustomVisioLocation.visio_key, EMPTY_STRING);
...
class AudioLocation extends ActionLocation {
	...
	side_action() {
		return new mel_html('a', { href: `tel:${this.audio} ; ${this.pin}#` })
			.generate()
			.click();
	}
}
...
class EventLocation extends BaseLocation {
	...
	_generate_locations(locations, event) {
		...
		for (let index = 0, len = locations.length, tmp; index < len; ++index) {
			...
			if (element.includes(AUDIO_URL))
				new_locations.audio = new AudioLocation(element);
			else if (CustomVisioLocation.is_custom_visio(element))
				new_locations.visio = new CustomVisioLocation(element);
