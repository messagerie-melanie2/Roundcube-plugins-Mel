	...
	rcmail.addEventListener('init', function(evt) {
			if (rcmail.env.task == 'settings'
				...
				rcmail.addEventListener('responseafterplugin.mel_doubleauth-removeuser', function(evt) {
					...
					$('#mel_doubleauth-form fieldset.main div.table div.row.added').remove();
				...
				rcmail.addEventListener('responseafterplugin.mel_doubleauth-adduser', function(evt) {
					...
					let table = document.querySelector('#mel_doubleauth-form fieldset.main div.table');
					...
					table.append(row(
						...
						col(rcmail.gettext('action_2', 'mel_doubleauth') + 
					...
					table.append(row(
						...
						col(rcmail.gettext('action_3', 'mel_doubleauth') + 
							'&nbsp;&nbsp;<input type="text" class="form-control"  size="32" id="2FA_code_input" value="' + secret + '">')
					));

					// Show QR Code 
					table.append(row(
						col('&nbsp;', 'col-sm-1') +
						col('<div id="2FA_qr_code" style="display: inline-block; margin-top: 10px; padding: 10px; background: #fff;"></div>')
					));

					// 4/ Code checker
					table.append(row(
						col(cercle(4), 'col-sm-1 my-auto') +
						col(rcmail.gettext('action_4', 'mel_doubleauth') + 
							'&nbsp;&nbsp;<input type="text" class="form-control" id="2FA_code_to_check" maxlength="10" onkeypress="if (event.keyCode == 13) return false;">' + 
							'&nbsp;&nbsp;<input type="button" class="button mainaction btn btn-primary" id="2FA_check_code" value="' + rcmail.gettext('check_code', 'mel_doubleauth') + '"/>')
					));
					...
					table.append(row(
						...
						col('<input type="button" class="button mainaction 2FA_save btn btn-primary" onclick="return rcmail.command(\'plugin.mel_doubleauth-save\',\'\',this,event)" value="Enregistrer" disabled="disabled">')
					));
					...
					let url_qr_code_values = 'otpauth://totp/'
