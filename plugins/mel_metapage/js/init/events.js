...
if (rcmail && window.mel_metapage) {
	...
	rcmail.addEventListener('toggle-quick-options.after', args => {
		...
		if (
			...
			if (
				'Large' === rcmail.env.mel_metapage_mail_configs['mel-scrollbar-size']
			) {
		...
		if (rcmail.env.task === 'settings') {
			...
			if (
				...
				if (true === rcmail.env.menu_last_frame_enabled) {
					...
					if (0 !== (mel_metapage.Frames.lastFrames?.length ?? 0))
	...
	rcmail.addEventListener('calendar.event_show_dialog.custom', async datas => {
		...
		if (event.categories !== undefined && event.categories.length > 0) {
			...
			if (isWsp) {
				...
				const wsp_name =
					mel_metapage.Storage.get(mel_metapage.Storage.title_workspaces, wsp)[
						wsp
					] ?? wsp;
					...
					.append(
						$(
							...
							if (typeof cloned_event.end !== 'string')
								cloned_event.end = cal.date2ISO8601(cloned_event.end.toDate());
