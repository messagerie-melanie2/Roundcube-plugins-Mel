...
}, ".tablink.mel.resources"); // pass the element as an argument to .on
      ...
      submit : function(e) {
        ...
        if ($('#rcmfd_changepassword_newpassword').val() != $('#rcmfd_changepassword_newpassword_confirm')
          ...
          rcmail.display_message(rcmail
              .gettext('mel_moncompte.error_password_confirm'), 'error');
  ...
  rcmail
      .addEventListener('init', function(evt) {
    	...
    	if (rcmail.env.enable_mesressources_mail) {
    		tab = $('<li>').attr('id', 'settingstabpluginmel_resources_bal')
	            ...
	            .attr('title', rcmail
	                .gettext('mel_moncompte.manageresourcesbal')).html(rcmail
	                .gettext('mel_moncompte.resourcesbal')).appendTo(tab);
	        // add tab
	        rcmail.add_element(tab, 'tabs');
    	}
    	...
    	if (rcmail.env.enable_mesressources_cal) {
    		tab = $('<li>')
	            ...
	            .attr('title', rcmail
	                .gettext('mel_moncompte.manageresourcesagendas'))
    	...
    	if (rcmail.env.enable_mesressources_addr) {
    		tab = $('<li>')
	            ...
	            .addClass('listitem_mel mel'), button = $('<a>')
	            .attr('href', rcmail.env.comm_path
	                + '&_action=plugin.mel_resources_contacts')
	            .attr('title', rcmail
	                .gettext('mel_moncompte.manageresourcescontacts'))
    	...
    	if (rcmail.env.enable_mesressources_task) {
    		tab = $('<li>')
	            ...
	            .addClass('listitem_mel mel'), button = $('<a>')
	            .attr('href', rcmail.env.comm_path
	                + '&_action=plugin.mel_resources_tasks')
	            .attr('title', rcmail
	                .gettext('mel_moncompte.manageresourcestaches'))
      ...
      // Ajout des ressources Mes applications du portail
        ...
        if (rcmail.env.action
          ...
          rcmail
              .register_command('plugin.mel_moncompte_delete_resource', function() {
        ...
        if (rcmail.env.enable_moncompte) {
        	tab = $('<li>').attr('id', 'settingstabpluginmel_moncompte')
	            ...
	            .attr('href', rcmail.env.comm_path
	                ...
	                .gettext('mel_moncompte.managemoncompte')).html(rcmail
	                .gettext('mel_moncompte.moncompte')).appendTo(tab);
        ...
        if (rcmail.env.action.indexOf('plugin.mel_moncompte') != -1 && rcmail.env.enable_moncompte) {
          var p = rcmail;

          if (rcmail.gui_objects.mel_moncompte_options_list) {
            rcmail.options_list = new rcube_list_widget(rcmail.gui_objects.mel_moncompte_options_list, {
              multiselect : false,
              draggable : false,
              keyboard : true
            });
        ...
        if (rcmail.env.enable_messtatistiques) {
            ...
            .attr('title', rcmail
                ...
                .gettext('mel_moncompte.statistics')).appendTo(tab);
        ...
        if (rcmail.env.enable_messtatistiques_mobile) {
        	tab = $('<li>')
	            ...
	            .attr('href', rcmail.env.comm_path
	                + '&_action=plugin.mel_statistics_mobile')
	            .attr('title', rcmail
	                .gettext('mel_moncompte.managestatisticsmobile'))
              ...
              .addEventListener('plugin.import_listes_csv_success', function(p) {
                ...
                rcmail
                    .display_message(rcmail
                        .gettext('mel_moncompte.listes_import_success'), 'success');
                if (p.addr_error.length > 0) {
                  alert(rcmail.gettext('mel_moncompte.listes_addr_error')
                      + p.addr_error);
                }
              });
        }
...
rcube_webmail.prototype.load_moncompte_frame = function(id) {
  ...
  if (this.env.contentframe && window.frames
    ...
    window.location.href = this.env.comm_path
        + '&_action=plugin.mel_moncompte' + (has_id ? '&_fid=' + id : '');
  }
};
...
rcube_webmail.prototype.add_resource = function() {
  if (rcmail.env.resources_action != 'bal') {
    ...
    var name = prompt(rcmail.gettext('mel_moncompte.add_resource_prompt_'
        + type));
    if (name) {
      var lock = this
          .display_message(rcmail.gettext('mel_moncompte.wait'), 'loading');
      this.http_post('plugin.mel_add_resource', {
        _name : name,
        _type : type
      }, lock);
...
rcube_webmail.prototype.delete_resource = function() {
  if (rcmail.env.resources_action != 'bal') {
    ...
    if (confirm(rcmail.gettext('mel_moncompte.delete_resource_confirm_'
        + type))) {
      var lock = this
          .display_message(rcmail.gettext('mel_moncompte.wait'), 'loading');
      this.http_post('plugin.mel_delete_resource', {
        _id : id,
        _type : type
      }, lock);
...
function AddExternalMember() {
  ...
  if (dn_list) {
    var newSmtp = prompt(rcmail
        .gettext('mel_moncompte.listes_memb_externe'), "");
    if (newSmtp) {
...
function RemoveMember() {
  ...
  if (dn_list) {
    ...
    if (address) {
      if (confirm(rcmail.gettext('mel_moncompte.listes_addr_del')
...
function refreshListMembers(dn_list) {
  ...
  if (lists_isdyn[dn_list]) {
  	$('.listes_buttons').hide();
  	$('#legende_list_membres').text(rcmail.gettext('mel_moncompte.listes_membres_dyn'));
  }
  else {	
  	$('.listes_buttons').show();
  	$('#legende_list_membres').text(rcmail.gettext('mel_moncompte.listes_membres'));
  } 
