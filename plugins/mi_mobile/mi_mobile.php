 ...
 *
 */
class mi_mobile extends rcube_plugin {
  /**
   *
   * @var string
   */
  ...
  function init() {
    ...
    $this->add_button(array('task' => 'switch_skin','data-ajax' => 'false', 'command' => 'switch_desktop','class' => 'button-switch_desktop ui-link ui-btn ui-corner-all ui-icon-desktop ui-btn-icon-left','innerclass' => 'button-inner','label' => 'mi_mobile.desktop'), 'taskbar_mobile');
    // Add the switch to mobile skin button
    if ($this->rc->config->get('skin') == 'mi_larry') {
      ...
      $this->add_button(array('task' => 'switch_skin','command' => 'switch_mobile','class' => 'about-link','classsel' => 'about-link button-selected','innerclass' => 'button-inner','label' => 'mi_mobile.mobile'), 'topline-left');
    }
    $this->register_action('switch_mobile', array($this,'switch_mobile'));
    $this->register_action('switch_desktop', array($this,'switch_desktop'));
    // Command
    $this->register_action('mi_mobile.set_current_page', array($this,'set_current_page'));
    ...
    if ($this->isMobile()) {
      if (in_array($this->rc->task, $this->rc->config->get("mobile_tasks", array('mail', 'addressbook', 'settings')))) {
        ...
        $this->include_stylesheet('css/mi_larry_mobile.min.css');
  ...
  public function isMobile() {
    ...
    $mobile_skin = $this->rc->config->get('skin') == 'mi_larry_mobile';
