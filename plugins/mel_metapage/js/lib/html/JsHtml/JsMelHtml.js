...
JsHtml.update('input', function (self, old, attribs = {}) {
    ...
    if (!['checkbox', 'button', 'color', 'file', 'image'].includes(attribs.type))  
    {
...
JsHtml.update('button', function (self, old, attribs = {}) {
    let html = old.call(self, attribs);
    return html.addClass('mel-button').addClass('no-button-margin').addClass('no-margin-button').css('border-style','solid');
});

JsHtml.extend('mel_button', function (attribs = {}) {
    return this.button(attribs).removeClass('no-button-margin').removeClass('no-margin-button');
});

JsHtml.update('text', function meltext(self, old, text, plugin = 'mel_metapage') {
    return old.call(self, rcmail.gettext(text, plugin));
});

JsHtml.update('accessibilty_setup_button', function (self, old, {isChild=false}) {
