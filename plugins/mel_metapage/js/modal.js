...
class GlobalModal {
  ...
  constructor(
    ...
    if (config.header === 'default') this.header.title.html(config.title);
    ...
    if (config.footer.exit === undefined && config.footer.save === undefined) {
      ...
      if (
        config.footer.save !== undefined &&
        config.footer.save.text !== 'default'
      )
        this.footer.buttons.save.html(config.footer.save.text);
      if (
        config.footer.save !== undefined &&
        config.footer.save.onclick !== undefined
      )
    ...
    $('.global-modal-header')
      ...
      .each((i, e) => {
        if (e.id === 'created-modal-title-div-auto') {
          if ($(e).find('h2').length === 0) $(e).remove();
        }
      });
  ...
  _create_reduced() {
    ...
    $(span)
      ...
      .append(
        $(button)
          .addClass('btn btn-secondary modal-minify-mel')
          .click(() => {
   ...
   */
  show() {
    if (this.isPhone()) this.setToPhone();
  ...
  setToPhone() {
    ...
    this.modal.find('.modal-content').css('border-radius', 0);
    ...
    this.footer.querry.css('text-align', 'center').css('display', 'block');
