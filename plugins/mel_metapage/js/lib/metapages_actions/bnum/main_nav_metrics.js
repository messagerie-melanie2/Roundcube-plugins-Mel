...
export class MainNavMetrics extends AMetricsModule {
  ...
  before(event) {
    ...
    if (!!function_called && tasks_action.includes(task))
      task = function_called.call(this, event, task) ?? task;
  ...
  _get_task_from_event(event) {
    return event.currentTarget.hasAttribute('data-task')
      ? event.currentTarget.getAttribute('data-task')
      : this._get_task(this._get_url(event));
  }
  ...
  async _send_current_task() {
    ...
    if (!current_tasks_to_ignore.includes(task)) {
      ...
      BnumLog.info('_send_current_task', 'current task : ', task, 'ignored !');
