(() => {
  function notes() {
    ...
    window.create_note = async () => {
      ...
      if (rcmail.env.mel_metapages_notes[default_note_uid] !== undefined)
        await rcmail.env.mel_metapages_notes[default_note_uid].post_add();
    ...
    top.mm_add_shortcut(
      'notes',
      Enumerable.from(rcmail.env.mel_metapages_notes)
        .orderBy((x) => x.value.order)
        .select((x) => Sticker.from(x.value).html())
        .toArray()
        .join(' '),
      true,
      'Notes',
    ...
    top.rcmail.addEventListener('apps.create', () => {
      ...
      if (rcmail.env.mel_metapages_notes_edited === true) {
        rcmail.triggerEvent('notes.master.update', {
          datas: rcmail.env.mel_metapages_notes,
        });
    ...
    top.rcmail.addEventListener('notes.master.edit', (args) => {
      ...
      return Sticker.from(rcmail.env.mel_metapages_notes[args.id])
        .post_update()
        .then(() => {
          rcmail.triggerEvent('notes.master.update', {
            datas: rcmail.env.mel_metapages_notes,
