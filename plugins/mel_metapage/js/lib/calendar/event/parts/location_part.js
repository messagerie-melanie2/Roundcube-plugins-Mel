...
export class LocationPartManager {
	...
	_on_select_changed(event) {
		...
		if (this._cached[id]?.[val]) {
			...
			this.locations[id] = new (LocationPartManager.PARTS.find(
				x => x.OptionValue() === val,
	...
	_update_selects(id) {
		...
		for (const iterator of $('.event-location-select')) {
			...
			if ($select.data('id') !== id) {
				const nb_infinity = MelEnumerable.from(LocationPartManager.PARTS)
				...
				if (
					...
					MelEnumerable.from(LocationPartManager.PARTS)
