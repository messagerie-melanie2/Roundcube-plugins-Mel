    ...
    if (rcmail.task == 'mail') {
      ...
      if (rcmail.env.action == 'compose') {
        ...
        var elem = $('#compose-attachments > div'),
          input = $(
            '<button class="button mel-button inverse" type="button"></button>',
          )
            .attr('tabindex', $('input', elem).attr('tabindex') || 0)
            .html(
              `<span class="icon-mel-link"></span> ${rcmail.gettext('roundrive.fromcloud')}`,
            )
            .click(function () {
              roundrive_selector_dialog();
            })
            .addClass('btn btn-secondary')
            .appendTo(elem);

        if (rcmail.gui_objects.filelist) {
          rcmail.file_list = new rcube_list_widget(
            rcmail.gui_objects.filelist,
            {
              multiselect: true,
              keyboard: true,
          ...
          rcmail.file_list
            .addEventListener('select', function (o) {
              roundrive_list_select(o);
            })
            .addEventListener('listupdate', function (e) {
              rcmail.triggerEvent('listupdate', e);
            });
        ...
        $.merge(rcmail.env.compose_commands, [
          'files-list',
          'files-sort',
          'files-search',
          'files-search-reset',
      ...
      // mail preview
        ...
        if ($('li', attachment_list).length) {
          var link = $('<a href="#" class="button filesaveall">')
            ...
            .click(function () {
              roundrive_directory_selector_dialog();
            })
...
*/
        ...
        rcmail.file_list.init();
        roundrive_list_coltypes();
        roundrive_drag_drop_init(
          $(rcmail.gui_objects.filelist).parents('.droptarget'),
        );
      }
...
function roundrive_directory_selector_dialog(id) {
  ...
  // Corrige le bug de défilement de la page chromium
  correct_render();
}

/**
 * Corrige un problème de défilement spécifique à Chromium.
 *
 * Cette fonction utilise un module externe appelé `Correctors` pour appliquer une correction
 * au comportement de défilement. Elle vérifie d'abord si le module `loadJsModule` est disponible
 * dans l'environnement global, puis charge dynamiquement le module `Correctors` depuis un chemin
 * spécifique. Si le module est chargé avec succès, la méthode `ScrollCorrector` est appelée.
 *
 * @async
 * @function correct_render
 * @returns {Promise<void>} Une promesse qui est résolue une fois la correction appliquée.
 */
async function correct_render() {
  // Vérifie si la fonction globale `loadJsModule` est disponible
  if (window.loadJsModule) {
    // Charge dynamiquement le module `Correctors` depuis le chemin spécifié
    const Correctors = (
      await loadJsModule('bnum', 'Correctors', '/js/lib/helpers/')
    )?.default;

    // Si le module `Correctors` est chargé avec succès, applique la correction
    if (Correctors) {
      await Correctors.ScrollCorrector();
    }
  }
}
...
function decode_text(text) {
  ...
  for (const iterator of chars) {
    text = text.replaceAll(iterator[0], iterator[1]);
  }
...
function roundrive_selector_dialog() {
  var dialog = $('#files-compose-dialog'),
    buttons = {};
  ...
  buttons[rcmail.gettext('roundrive.attachsel')] = function () {
    ...
    if (list.length) {
      ...
      var content =
          '<span>' + rcmail.get_label('roundrive.attaching') + '</span>',
        id = new Date().getTime();
  ...
  buttons[rcmail.gettext('roundrive.linksel')] = async function () {
    ...
    await $.ajax({
      type: 'POST',
      url: rcmail.url('plugin.roundrive'),
      data,
      dataType: 'json',
      success: function (data) {
        ...
        const text = `${return_symbol}/!\\Pour que le destinataire accède au fichier partagé par lien, assurez-vous qu'il dispose des droits nécessaires.`;
        const INDICATOR =
          '-- ' + rcmail.gettext('roundrive.shareddocuments') + ' :';
        const END = `${return_symbol}//---------------------//`;
        if (typeof data === 'string') data = JSON.parse(data);

        let content = decode_text(rcmail.editor.get_content());

        if (!content.includes(INDICATOR))
          content = INDICATOR + text + END + content;
        ...
        for (const iterator of data) {
          var url = rcmail.get_task_url(
            `stockage&_params=${encodeURIComponent('/f/' + iterator.id)}`,
            window.location.origin + window.location.pathname,
          );
          html += is_html
            ? `${return_symbol}<a href="${url}">${iterator.name}</a>`
            : `${return_symbol}${url}`;
        }
  ...
  roundrive_dialog_show(dialog, {
    ...
    buttons: buttons,
    button_classes: [
      'mainaction mel-button button-white float-right icon-mel-arrow-right-after',
      'mel-button button-white float-right icon-mel-arrow-right-after',
      'mel-button button-white float-left icon-mel-undo-after',
    ],
    minWidth: 500,
    minHeight: 300,
    width: 700,
  ...
  args.buttons[rcmail.gettext('roundrive.save')] = function () {
    ...
    $('#source-' + driver + ' input').each(function () {
      if (this.name.startsWith(driver + '[')) {
        args[this.name.substring(driver.length + 1, this.name.length - 1)] =
          this.value;
      }
    });
...
function roundrive_file_edit_dialog(file) {
  var dialog = $('#files-file-edit-dialog'),
    buttons = {},
    name = file_api.file_name(file);
  input = $('input[name="name"]', dialog).val(name);

  buttons[rcmail.gettext('roundrive.save')] = function () {
    var folder = file_api.file_path(file),
      name = input.val();
...
function roundrive_dialog_show(content, params, onopen) {
  ...
  params.close = function (e, ui) {
    ...
    stack.pop();
    if (stack.length) {
      elem = stack[stack.length - 1].find(
        'input[type!="hidden"]:not(:hidden):first',
      );
      if (!elem.length)
        elem = stack[stack.length - 1]
          .parent()
          .find('a[role="button"], .ui-dialog-buttonpane button')
          .first();
    }
  ...
  content
    .appendTo(dialog)
    .show()
    .find('input[type!="hidden"]:not(:hidden):first')
    .focus();
...
function roundrive_upload_input(button) {
  ...
  file
    ...
    .change(function () {
      rcmail.files_upload('#filesuploadform');
    })
    .click(function () {
      setTimeout(function () {
        link.mouseleave();
  ...
  link
    .css({ overflow: 'hidden', cursor: 'pointer', position: 'relative' })
    .mouseenter(function () {
      this.__active = true;
    })
...
roundrive_set_coltypes = function (list) {
  var i,
    found,
    name,
    cols = list.list.tHead.rows[0].cells;
...
roundrive_list_keypress = function (list) {
  ...
  if (list.key_pressed == list.ENTER_KEY) rcmail.command('files-open');
    ...
    list.key_pressed == list.DELETE_KEY ||
    list.key_pressed == list.BACKSPACE_KEY
  )
...
roundrive_drag_end = function (e) {
  ...
  if (folder.length) {
    ...
    if (menu && modkey == SHIFT_KEY && rcmail.commands['files-copy']) {
      ...
      $(menu)
        .css({ top: pos.y - 10 + 'px', left: pos.x - 10 + 'px' })
        .show();
      return;
    }
...
roundrive_frame_load = function (frame) {
  ...
  rcmail.file_editor =
    win.file_editor && win.file_editor.editable ? win.file_editor : null;

  if (rcmail.file_editor) rcmail.enable_command('files-edit', true);

  rcmail.enable_command(
    'files-print',
    (rcmail.file_editor && rcmail.file_editor.printable) ||
...
roundrive_drag_drop_init = function (container) {
  ...
  container.get(0).addEventListener(
    'drop',
    function (e) {
...
roundrive_progress_str = function (param) {
  var current,
    total = file_api.file_size(param.total).toUpperCase();
...
rcube_webmail.prototype.files_sort = function (props) {
  ...
  if (!this.env.disabled_sort_order)
    sort_order =
      this.env.sort_col == sort_col && sort_order == 'ASC' ? 'DESC' : 'ASC';

  // set table header and update env
  this.set_list_sorting(sort_col, sort_order);

  this.http_post('files/prefs', {
    roundrive_sort_col: sort_col,
    roundrive_sort_order: sort_order,
  });
...
rcube_webmail.prototype.files_set_quota = function (p) {
  if (p.total && window.file_api) {
    ...
    p.total *= 1024;
    p.title =
      file_api.file_size(p.used) +
      ' / ' +
      file_api.file_size(p.total) +
      ' (' +
      p.percent +
      '%)';
  }
...
*/
  ...
  this.folder_list_response = function (response) {
    ...
    var first,
      ...
      parent_id =
        response.parent && this.env.folders
          ? this.env.folders[response.parent].id
          : '';
    ...
    if (!elem.length && window.parent && parent.rcmail) {
      ...
      is_new_list = $(
        '#files-folder-list ul#folders' + parent_id,
      ...
      list = is_new_list
        ? $(
            '<ul id="folders' +
              parent_id +
              '" class="treelist listing iconized" role="tree"></ul>',
          )
        : $(
            '#files-folder-list ul#folders' + parent_id,
            window.parent.document.body,
      ...
      is_new_list = $('#files-folder-list ul#folders' + parent_id).length
        ? false
        : true;
      list = is_new_list
        ? $(
    ...
    var folders = this.folder_list_parse(
  ...
  this.folder_select = function (folder, is_collection) {
    ...
    if (is_collection) {
      ...
      var found = $('#' + this.env.folders[folder].id, list).addClass(
        'selected',
      );
  ...
  this.folder_toggle = function (folder) {
    ...
    if (
      elem
        .find('ul li#' + folder_id + ' > div.treetoggle')
        .hasClass('collapsed')
    ) {
  ...
  this.folder_list_row = function (i, folder) {
    var row = $(
      '<li class="mailbox" role="treeitem"><span class="branch"></span></li>',
    );
    ...
    row
      ...
      .append(
        $('<ul class="treelist listing iconized"></ul>').attr(
          'id',
          'folders' + folder.id,
        ),
      );
    ...
    if (folder.virtual) {
      ...
      row.find('> div.treetoggle').click(function (e) {
        e.stopPropagation();
      ...
      row
        .attr('tabindex', 0)
        .keypress(function (e) {
          if (e.which == 13 || e.which == 32) file_api.folder_select(i);
        })
        .click(function (e) {
          e.stopPropagation();
          file_api.folder_select(i);
        })
        .mouseenter(function () {
          if (
            rcmail.file_list &&
            rcmail.file_list.drag_active &&
  ...
  this.file_list = function (params) {
    ...
    if (params.collection || params.all_folders) this.file_list_loop(params);
      ...
      this.requests[params.req_id] = this.request(
        'file_list',
        params,
        'file_list_response',
      );
  ...
  this.file_list_loop_result_add = function (result) {
    ...
    if (this.env.folders_loop_lock) {
      setTimeout(function () {
        file_api.file_list_loop_result_add(result);
      }, 100);
      return;
    }
  ...
  this.file_list_row = function (file, data, index) {
    ...
    for (c in rcmail.env.coltypes) {
      ...
      if (c == 'name')
        col =
          '<td class="name filename ' +
          this.file_type_class(data.type) +
          '">' +
          '<span>' +
  ...
  this.file_get = function (file, params) {
    ...
    rcmail.redirect(
      rcmail.url('roundrive/file_api') + '&method=file_get&file=' + file,
    );
  };
  ...
  this.file_delete_response = function (response) {
    ...
    if (rcmail.env.file) {
      ...
      if (
        rco &&
        rco.file_list &&
        (opener.file_api.env.folder == dir || !opener.file_api.env.folder)
      )
  ...
  this.file_move = function (files, folder) {
    ...
    $.each(files, function (i, v) {
      var name =
        folder + file_api.env.directory_separator + file_api.file_name(v);
  ...
  this.file_move_response = function (response) {
    ...
    if (
      ...
      response.result.already_exist.length
    )
  ...
  this.file_move_ask_user = function (list, move) {
    var file = list[0],
      buttons = {},
      text = rcmail
        .gettext('roundrive.filemoveconfirm')
        .replace('$file', file.dst);
    dialog = $('<div></div>');
    ...
    buttons[rcmail.gettext('roundrive.fileoverwrite')] = function () {
      ...
      file_api.req = file_api.set_busy(
        true,
        move ? 'roundrive.filemoving' : 'roundrive.filecopying',
      );
      file_api.request(
        action,
        { file: f, overwrite: 1 },
        'file_move_ask_user_response',
      );
    };
  ...
  this.file_move_ask_user_response = function (response) {
    var move = this.file_move_ask_mode,
      list = this.file_move_ask_list;
    ...
    if (list && list.length) this.file_move_ask_user(list, mode);
      ...
      this.display_message(
        'roundrive.file' + (move ? 'move' : 'copy') + 'notice',
        'confirmation',
      );
  ...
  this.file_upload = function (form) {
    ...
    this.file_upload_form(form, 'file_upload', function (event) {
      ...
      try {
        ...
        if (
          response.slice(0, 5).toLowerCase() == '<pre>' &&
          response.slice(-6).toLowerCase() == '</pre>'
        ) {
  ...
  this.file_upload_size_check = function (files) {
    var i,
      size = 0,
      maxsize = rcmail.env.files_max_upload;
    ...
    if (maxsize && files) {
      ...
      if (size > maxsize) {
        alert(
          rcmail
            .get_label('roundrive.uploadsizeerror')
            .replace('$size', roundrive_file_size(maxsize)),
        );
        return false;
      }
    }
  ...
  this.file_upload_form = function (form, action, onload) {
    ...
    if (document.all) {
      var html =
        ...
        '"' +
        ' src="' +
        rcmail.assets_path('program/resources/blank.gif') +
        '"' +
    ...
    $(form)
      .attr({
        ...
        action:
          this.env.url +
          this.url(action, { folder: this.env.folder, token: this.env.token }),
        method: 'POST',
      })
  ...
  this.file_drop = function (e) {
    ...
    var submit_data = function () {
      ...
      $.ajax({
        ...
        url:
          file_api.env.url +
          file_api.url('file_upload', { folder: file_api.env.folder }),
    ...
    if (rcmail.env.files_progress_name) {
      if (formdata) formdata.append(rcmail.env.files_progress_name, ts);
        ...
        multipart +=
          ...
          rcmail.env.files_progress_name +
          '"' +
          crlf +
          crlf +
          ts +
      ...
      // use FileReader supporetd by Firefox 3.6
        ...
        reader.onload = (function (file, j) {
          return function (e) {
            ...
            multipart +=
              '; filename="' + (f.name_bin || file.name) + '"' + crlf;
  ...
  this.file_save = function (file, content) {
    ...
    this.request(
      'file_update',
      { file: file, content: content, info: 1 },
      'file_save_response',
    );
  };
  ...
  this.folder_auth_response = function (response) {
    ...
    $.each(this.env.folders, function () {
      cnt++;
    });
  ...
  this.folder_list_auth_form = function (driver) {
    ...
    $.each(driver.form, function (fi, fv) {
      var id = 'authinput' + fi,
        attrs = {
          type: fi.match(/pass/) ? 'password' : 'text',
          size: 25,
          name: fi,
          id: id,
        },
