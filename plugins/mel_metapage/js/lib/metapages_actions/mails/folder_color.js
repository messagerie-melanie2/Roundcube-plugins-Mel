...
export class FolderColor extends AFolderModifier {
    ...
    this.rcmail().register_command(
      ...
      () => {
        ...
        this._input_color = JsHtml.start
          .input_color({
            id: 'color-folder-changer',
            value: color,
            oninput: this._preview.bind(this, folder),
            onchange: this.set_to_server.bind(this, folder, color),
          })
          .css('display', 'none')
          .generate()
          .appendTo($('body'));
  ...
  async set_to_server(
    folder,
    start_color,
    updated_color = FolderColor.DEFAULT_COLOR,
  ) {
    ...
    const data = await BnumConnector.connect(
      BnumConnector.connectors.mail_set_folder_color,
      { params: config },
    );
  ...
  async get_from_server() {
    return await BnumConnector.connect(
      BnumConnector.connectors.mail_get_folders_color,
      {},
    );
  }
