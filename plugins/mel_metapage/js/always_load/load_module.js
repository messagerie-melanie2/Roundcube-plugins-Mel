(() => {
    ...
    if (!modules[key]) {
      ...
      try {
        modules[key] = await import(
          `../../../${plugin}${path}${name}?v=${VERSION}`
        );
      } catch (error) {
        console.error(
          `###[loadJsModule]Impossible de charger le module ${key}`,
          error,
        );
        throw error;
      }
  ...
  async function loadAction(
    ...
    if (!actions[key]) {
      const Wait = (await loadJsModule('mel_metapage', 'mel_promise'))
        .WaitSomething;
  ...
  async function load_js_page(
    name,
    { plugin = 'mel_metapage', skin = window?.rcmail?.env?.skin ?? '' },
  ) {
    var { JsHtml } = await loadJsModule(
      'mel_metapage',
      'JsHtml.js',
      '/js/lib/html/JsHtml/',
    );
