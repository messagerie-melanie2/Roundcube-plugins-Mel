    ...
    .on("pageshow", ".jqm-message", function() {
      ...
      if (rcmail.env.isenigma) {
        if (('#messagebody div.enigmaerror').length) {
          $('#messagebody div.enigmaerror').html($('#messagebody div.enigmaerror').html() + ' ' + rcmail.gettext('mi_mobile.addenigmapass'));
...
$(document).ready(function() {
  ...
  // Quick search box
  $('#quicksearchbox').attr('placeholder', rcmail.get_label('mi_mobile.search_in_calendars'));
...
if (window.rcmail) {
  ...
  rcmail.addEventListener('responseafterlist', function(evt) {
    ...
    rcmail.env.current_page = 1;
    rcmail.http_post('mi_mobile.set_current_page', {});
  });
}
