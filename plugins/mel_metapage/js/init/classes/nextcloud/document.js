var nextcloud_document = (() => {
  ...
  class Nextcloud_Icon {
    ...
    constructor(templateIcon, trueIcon, type = null) {
      ...
      this.icon = trueIcon;
      if (templateIcon === null && type !== null) this.init(type);
    }
    ...
    async init(type) {
      ...
      for (
        let index = 0,
          len = window?.rcmail?.env?.mel_metapage_templates_doc?.length ?? 0;
        index < len;
        index++
      ) {
  ...
  class Nextcloud_Document {
    ...
    addRaw(type, iconClass, href) {
      this.datas.push(
        new Nextcloud_Document_Config(type, null, iconClass, href, null),
      );
    }
    ...
    addIcon(template, _class) {
      this.datas.push(
        new Nextcloud_Document_Config(null, template, _class, null, null),
      );
    }
     ...
     */
    getIcon(icon) {
      if (this.datas.length === 0) return icon;
