...
class annuaire extends rcube_plugin
{
        ...
        if ($this->rc->task == 'addressbook' && !isset($this->base_dn) && !isset($search) && !isset($find)) {
          ...
          if ($this->source == $this->rc->config->get('annuaire_source', null)) {
              ...
              foreach ($contacts->list_groups($search, $search_mode) as $group) {
                ...
                if ($group['email']) {
                  ...
                  foreach ((array)$group['email'] as $i => $email) {
                    ...
                    $html = $this->get_html(['name' => $name, 'description' => $email, 'class' => $classname, 'title' => $email_format]);
                    $elements[] = array('id' => $row_id, 'email' => $email, 'mail' => $email_format, 'classes' => [$classname, 'object'], 'html' => $html);
                  }
                }
                // show group with count
                  ...
                  if (!empty($name)) {
                    $html = $this->get_html(['name' => $name, 'description' => intval($result->count), 'class' => $classname, 'title' => $name]);
                    $elements[] = array('id' => $row_id, 'mail' => null, 'name' => $name,'classes' => [$classname, 'object'], 'html' => $html);
                  }
                }
              }
              $contacts->reset();
            ...
            if (isset($search)) {
              ...
              $result = $contacts->search(['name', 'email'], $search, $search_mode);
            }
            else {
          ...
          if (isset($search) && $this->rc->task == 'addressbook') {
            echo json_encode(['action' => 'plugin.annuaire', 'elements' => $elements, 'source' => $this->source, 'unlock' => $unlock]);
          }
          else if (isset($find)) {
            echo json_encode(['action' => 'plugin.annuaire', 'find' => rcube_ldap::dn_encode($find) . '-' . $this->source, 'elements' => $elements, 'unlock' => $unlock]);
          }
          else {
            echo json_encode(['action' => 'plugin.annuaire', 'elements' => $elements, 'search' => isset($search), 'id' => $id, 'unlock' => $unlock]);
          }
          exit;
        }
        else {
          ...
          foreach ($this->rc->get_address_sources(false, true) as $j => $source) {
            ...
            if ($id == $this->rc->config->get('annuaire_source', null)) {
              ...
              $classes = ['folder', 'container', 'annuaire'];
            }
            else {
              $classes = ['folder', 'container', 'addressbook'];
            }
            $html = $this->get_html(['id' => $id, 'name' => $source['name'], 'description' => '', 'class' => 'folder', 'title' => $source['name']]);
            $elements[] = ['id' => $id, 'dn' => $base_dn, 'classes' => $classes, 'collapsed' => true, 'html' => $html, 'children' => [['id' => $id . '-child', 'classes' => ['child'], 'html' => '<span></span>']]];
          }
          // send output
          header("Content-Type: application/json; charset=".RCUBE_CHARSET);
          // Return the result to the ajax command
          echo json_encode(['action' => 'plugin.annuaire', 'elements' => $elements, 'unlock' => $unlock]);
          exit;
        }
    }
    ...
    private function get_elements($search = false) {
        $folders = []; $lists = []; $persons = []; $applications = [];
        $services = []; $functions = []; $units = []; $resources = [];
        $this->ldap->anonymous();
        $attributes = ['objectclass', 'ou', 'cn', 'description', 'mineqportee', 'mineqtypeentree', 'mineqordreaffichage', 'mineqpublicationphotointranet', 'mailpr'];
        if ($search) {
        ...
        if ($sr !== false) {
            ...
            foreach ($infos as $info) {
                ...
                $name = ''; $description = ''; $class = ''; $order = ''; $title = '';
                ...
                foreach ($info['objectclass'] as $k => $v) {
                    switch ($v) {
                        ...
                        case 'mineqMelListeAbonnement':
                            ...
                            $class = 'list';
                            $html = $this->get_html(['name' => $name, 'description' => $description, 'class' => $class, 'title' => $title, 'gototree' => $search]);
                            $lists[] = array('id' => $id, 'dn' => $info['dn'], 'email' => $email, 'mail' => format_email_recipient($email, $name), 'classes' => [$class, 'object'], 'order' => $order, 'html' => $html);
                            break;
                        case 'organizationalUnit':
                            ...
                            $class = 'folder';
                            $html = $this->get_html(['name' => $name, 'description' => $description, 'class' => $class, 'title' => $title, 'gototree' => $search]);
                            $folders[] = array('id' => $id, 'dn' => $info['dn'], 'email' => $email, 'classes' => [$class], 'order' => $order, 'collapsed' => true, 'html' => $html, 'children' => [['id' => $id . '-child', 'classes' => ['child'], 'html' => '<span></span>']]);
                            break;
                        case 'mineqMelBoite':
                        case 'mineqMelDP':
                            $name = $info['cn'][0];
                            ...
                            switch ($info['mineqtypeentree'][0]) {
                              ...
                              default:
                                if (isset($info['mineqpublicationphotointranet'][0]) && $info['mineqpublicationphotointranet'][0]) {
                                  ...
                                  $classes = ['person', 'nophoto', 'object'];
                                }
                                $html = $this->get_html(['name' => $name, 'description' => $description, 'class' => $class, 'title' => $title, 'gototree' => $search]);
                                $persons[] = array('id' => $id, 'dn' => $info['dn'], 'email' => $email, 'mail' => format_email_recipient($email, $name), 'classes' => $classes, 'order' => $order, 'html' => $html);
                                break;
                              case 'BALA':
                                $class = 'application';
                                $html = $this->get_html(['name' => $name, 'description' => $description, 'class' => $class, 'title' => $title, 'gototree' => $search]);
                                $applications[] = array('id' => $id, 'dn' => $info['dn'], 'email' => $email, 'mail' => format_email_recipient($email, $name), 'classes' => [$class, 'object'], 'order' => $order, 'html' => $html);
                                break;
                              case 'BALS':
                                $class = 'service';
                                $html = $this->get_html(['name' => $name, 'description' => $description, 'class' => $class, 'title' => $title, 'gototree' => $search]);
                                $services[] = array('id' => $id, 'dn' => $info['dn'], 'email' => $email, 'mail' => format_email_recipient($email, $name), 'classes' => [$class, 'object'], 'order' => $order, 'html' => $html);
                                break;
                              case 'BALF':
                                $class = 'function';
                                $html = $this->get_html(['name' => $name, 'description' => $description, 'class' => $class, 'title' => $title, 'gototree' => $search]);
                                $functions[] = array('id' => $id, 'dn' => $info['dn'], 'email' => $email, 'mail' => format_email_recipient($email, $name), 'classes' => [$class, 'object'], 'order' => $order, 'html' => $html);
                                break;
                              case 'BALU':
                                $class = 'unit';
                                $html = $this->get_html(['name' => $name, 'description' => $description, 'class' => $class, 'title' => $title, 'gototree' => $search]);
                                $units[] = array('id' => $id, 'dn' => $info['dn'], 'email' => $email, 'mail' => format_email_recipient($email, $name), 'classes' => [$class, 'object'], 'order' => $order, 'html' => $html);
                                break;
                              case 'BALR':
                                $class = 'resource';
                                $html = $this->get_html(['name' => $name, 'description' => $description, 'class' => $class, 'title' => $title, 'gototree' => $search]);
                                $resources[] = array('id' => $id, 'dn' => $info['dn'], 'email' => $email, 'mail' => format_email_recipient($email, $name), 'classes' => [$class, 'object'], 'order' => $order, 'html' => $html);
                                break;
                            }
    ...
    private function get_html($object) {
      $content = html::span(['id' => 'l:' . $object['id'], 'class' => 'name', 'title' => $object['title']], $object['name']);
      $content .= html::span(['class' => 'description'], $object['description']);
      if ($object['gototree']) {
        $content .= html::div([
    ...
    private function get_user_service($uid) {
      ...
      if ($ldap->anonymous()) {
        do {
          ...
          $base_dn = $tmp[1];
          // Search LDAP
          $result = $ldap->ldap_list($base_dn, $filter, ['cn']);
