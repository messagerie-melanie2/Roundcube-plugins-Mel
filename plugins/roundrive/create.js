...
RoundriveCreate.prototype.create_buttons = function () {
  ...
  for (
    ...
    html +=
      '<div class=col-md-3><button data-doc="' +
      element.type +
      '" type=button class="doc-' +
      element.type +
...
RoundriveCreate.prototype.create_document = function (goFunc = null) {
  ...
  if (values.type === undefined) {
    mel_metapage.Functions.call(
      'rcmail.display_message("Vous devez choisir un type de document !", "error")',
    );
    this.item.find('.first').addClass('error');
    $continue = false;
  } else this.item.find('.first').removeClass('error');

  if (values.name === '') {
    mel_metapage.Functions.call(
      'rcmail.display_message("Vous devez entrer un nom !", "error")',
    );
    this.inputs.name.addClass('error');
    $continue = false;
  } else this.inputs.name.removeClass('error');
  ...
  if (fullpath.length >= 255) {
    ...
    mel_metapage.Functions.call(
      'rcmail.display_message("Impossible de mettre le fichier ici !", "error")',
    );
    this.inputs.name.addClass('error');
    $continue = false;
  }
    ...
    $return = mel_metapage.Functions.post(
      ...
      (datas) => {
        ...
        if (datas.success) {
          ...
          mel_metapage.Functions.call(
            'create_popUp.close(); create_popUp = undefined',
          );
          mel_metapage.Functions.call(
            `rcmail.display_message("${datas.error}", "error")`,
          );
        }
      },
    );
  ...
  if (elements.chevron.length > 0) {
    ...
    if (elements.querry.find('ul').length > 0) {
      ...
      elements.chevron
        ...
        .addClass(chevron.loading)
        .addClass('mel-loading');
      ...
      returns = mel_metapage.Functions.get(
        ...
        (datas) => {
          ...
          if (datas.length === 0)
            elements.chevron
              .removeClass(chevron.loading)
              .removeClass('mel-loading')
              .addClass(chevron.none)
              .removeClass('mel-clickable');
          else {
            elements.chevron
              .removeClass(chevron.loading)
              .removeClass('mel-loading')
            ...
            for (let index = 0; index < datas.length; ++index) {
              ...
              const path =
                `${rcmail.gettext('files', 'roundrive')}/${decodeURIComponent(element.path)}`.replaceAll(
                  '"',
                  '¤¤¤',
                );
