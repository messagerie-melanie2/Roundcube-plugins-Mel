$(document).ready(async () => {
    ...
    script.onerror = function (e, a, b) {
      if (setTwitterSrc.first === undefined) {
        rcmail.display_message(
          rcmail.gettext('twitter_navigator_error', plugin_text),
          'error',
        );
      ...
      for (let index = 0; index < MelCustomNews.allCustomNews.length; index++) {
        ...
        if (element.type === 'twitter') {
          element.$news
            .find('.square-contents .headlines-title')
            .css('display', 'block')
            .html(`Twitter de ${element.id}`);
          element.$news
            .find('.square-contents .headlines-contents')
            .css('display', 'block')
            .html(
  ...
  class NewsPopup {
    constructor() {
      ...
      const _GlobalModalConfig = this.isSettings()
        ? parent.GlobalModalConfig
        : GlobalModalConfig;
      ...
      this.modal.show = function () {
        ...
        if ($('#created-modal-title-div-auto').length > 0)
          this.removeBeforeTitle();
     ...
     */
    static fabric() {
      if (NewsPopup.cache === undefined) NewsPopup.cache = new NewsPopup();
    ...
    drawChoice(title, ...choices) {
      ...
      for (const key in choices) {
        if (Object.hasOwnProperty.call(choices, key)) {
          ...
          $html.append(
            $(`<button style="margin-top:0px;margin-right:15px" class="btn btn-block ${element.classes === undefined ? 'btn-secondary' : element.classes} btn-mel">
    ...
    createOrEditPublish(id = null, isFlux = false) {
      ...
      if (rcmail.env.news_current_news_datas !== undefined) {
        ...
        if (
          rcmail.env.news_service_for_publish[
            rcmail.env.news_current_news_datas.service
          ] === undefined
        )
          rcmail.env.news_service_for_publish[
            rcmail.env.news_current_news_datas.service
        ...
        if (
          rcmail.env.news_service_for_publish[news.type] === undefined &&
          news.type !== null &&
          news.type !== undefined &&
          news.type !== ''
        )
          rcmail.env.news_service_for_publish[news.type] = news.type
            .split(',', 2)[0]
            .split('=')[1];
      }
      ...
      //Titre de la modale
      if (id === null)
        this.modal.editTitle(rcmail.gettext('create_publish', plugin_text));
      else
        this.modal.editTitle(
          `${rcmail.gettext('edit', plugin_text)} "${news.title}"`,
        );
      ...
      let html =
        obFields +
        '<div><div class="row"><div class="service-left col-12">' +
        this.createSelect(
          rcmail.gettext('choose_service', plugin_text),
          'mel-publish-service',
          rcmail.gettext('select_service', plugin_text),
          Enumerable.from(rcmail.env.news_service_for_publish)
            .select((x) => {
              return { value: x.key, text: x.value };
            })
            .toArray(),
          id === null ? 'none' : news.getService(),
        ) +
        '</div><div class="service-right hidden"><button id="np-select-child" class="mel-button btn btn-secondary" style="margin-top: 53px;">Choisir un sous-service</button></div></div></div>';
      ...
      if (!isFlux) {
        html +=
          '<div>' +
          this.createInput(
            rcmail.gettext('choose_title', plugin_text),
            'mel-publish-title',
            'text',
            rcmail.gettext('admin_news_title', plugin_text),
            news.title,
            'mel-publish-title',
            classes,
          ) +
        ...
        html +=
          '<div>' +
          this.createTextarea(
            rcmail.gettext('write_admin_news', plugin_text),
            'mel-publish-body',
            rcmail.gettext('admin_news_placeholder', plugin_text),
            news.body,
          ) +
          '</div>';
        ...
        html += this.createSelect(
          rcmail.gettext('choose_intranet', plugin_text),
          'news-intranet-select',
          rcmail.gettext('select_website', plugin_text),
          Enumerable.from(rcmail.env.news_intranet_list)
            .select((x) => {
              return { value: x.key, text: x.value.name };
            })
            .toArray(),
          id === null ? 'none' : news.id,
        );
      }

      //Modification de l'affichage de la modale (Voir la classe modal de mel_metapage)
      this.modal.editBody(
        '<div class="np-content mel-r">' +
          html +
          '</div><div class="np-other mel-r"></div>',
      );
      ...
      this.modal.modal.find('#publish-delete-button').click(() => {
        if (confirm(rcmail.gettext('delete_confirm', plugin_text))) {
          ...
          rcmail.display_message(
            rcmail.gettext('deleting', plugin_text),
            'loading',
          );
          this.post(
            isFlux ? 'delete_rss' : 'delete',
            {
            ...
            (datas) => {
              ...
              if (datas === 'denied') {
                rcmail.display_message(
                  rcmail.gettext('action_denied', plugin_text),
                  'error',
                );
              } else {
                rcmail.display_message(
                  rcmail.gettext('succefuly_published', plugin_text),
                  'confirmation',
                );
      ...
      this.modal.modal.find('#mel-publish-service').on('change', () => {
        ...
        if (
          !service_right.hasClass(col_class_right) &&
          this.modal.modal.find('#mel-publish-service').val() !== 'none'
        ) {
          service_right.addClass(col_class_right).removeClass('hidden');
          this.modal.modal
            .find('.service-left')
            .removeClass('col-12')
            .addClass(`col-${col_left}`);
          this.modal.modal.find('#np-select-child').click(() => {
            ...
            new AnnuaireTree(this.modal.modal.find('.np-other'))
              .setAction(
                AnnuaireTree.actionsList.onGetAddress,
                (arbre, service) => {
              ...
              .setAction(
                AnnuaireTree.actionsList.htmlButton,
                (arbre, html, service, datas) => {
                  const end = ',dc=equipement,dc=gouv,dc=fr';
                  service = datas.dn;

                  if (!service.includes(end)) service += end;
                  ...
                  $querry.remove();
                  $querry = html.append(
                    `<button data-service="${service}" class="mel-button no-button-margin true bckg nbtn-action">${datas.dn.replace(',dc=equipement,dc=gouv,dc=fr', '') === this.modal.modal.find('#mel-publish-service').val() ? '<span class="icon-mel-check icon"></span>' : '<span class="icon"></span>'}${text}</button>`,
                  );
                  return html[0].outerHTML;
                },
              )
              .setAction(
                AnnuaireTree.actionsList.afterShowChildren,
                (arbre, datas, service, $button, $span) => {
                  if (
                    !Enumerable.from(datas.elements).any(
                      (x) => x.classes.length > 0 && x.classes[0] === 'folder',
                    )
                  ) {
                    $span
                      .removeClass(arbre.icons.opened)
                      .addClass(arbre.icons.empty)
                      .parent()
                      .parent()
                      .addClass('disabled')
              ...
              .setAction(
                AnnuaireTree.actionsList.onNotFolderClick,
                (arbre, service, $button) => {
                  arbre.panel
                    .find('button.nbtn-action .icon')
                    .removeClass('icon-mel-check');
              ...
              .setAction(AnnuaireTree.actionsList.afterSetup, (arbre) => {
                arbre.panel.append(
                  $(
                    `<button class="added-undo-button mel-button btn btn-primary">${rcmail.gettext('back')} <span class="plus icon-mel-undo"></span></button>`,
                  ).click(() => {
                ...
                if (arbre.panel.find('li').length === 0) {
                  ...
                  rcmail.display_message(
                    rcmail.gettext('no_sub_services', plugin_text),
                  );
                } else {
                  arbre.panel.append(
                    $(
                      `<button style="float:right" class="mel-button btn btn-primary">${rcmail.gettext('validate', plugin_text)} <span class="plus icon-mel-check"></span></button>`,
                    ).click(() => {
                      const selected = arbre.panel.find(
                        'button.nbtn-action .icon-mel-check',
                      );
                      if (selected.length > 0) {
                        const btn = selected.parent();
                        if (
                          this.modal.modal
                            .find('#mel-publish-service')
                            .find(`option[value="${btn.data('service')}"]`)
                            .length > 0
                        )
                          this.modal.modal
                            .find('#mel-publish-service')
                            .val(btn.data('service'));
                        else {
                          this.modal.modal
                            .find('#mel-publish-service')
                            .append(
                              `<option value=${btn.data('service')}>${btn.find('.name').html()} - ${btn.find('.description').html()}</option>`,
                            )
                            .val(btn.data('service'));
                  ...
                  arbre.panel.prepend(
                    `<h4>${rcmail.gettext('choose_sub_service', plugin_text)}</h4>`,
                  );
                }
              })
      ...
      if (!isFlux) {
        ...
        $(
          `<button class="mel-button" style="margin-right:15px">${rcmail.gettext('visualize', plugin_text)} <span class="plus icon-mel-${news.id === '' ? 'plus' : 'pencil'}"></span></button>`,
        )
          .click(() => {
            if (
              this.check(
              ...
              this.confirm('createOrEditPublish', {
                id: 'news-generated-tmp-id',
                trueId: news.id,
              });
          })
          .appendTo(this.modal.footer.querry);

        $('label[for="mel-publish-body"]')
          .css('display', 'inline-block')
          .css('width', 'auto')
          .parent()
          .append(
            `<div id=div-publish-notify style="display:none;/*inline-block*/;float:right;margin-top:15px;"><input id=publish-notify type=checkbox /> <label for="publish-notify">${rcmail.gettext('ask_notify', plugin_text)}</label></div>`,
          );
        ...
        $(
          `<button class="mel-button" style="margin-right:15px">${news.id === '' ? rcmail.gettext('publish', plugin_text) : rcmail.gettext('edit', plugin_text)} <span class="plus icon-mel-${news.id === '' ? 'plus' : 'pencil'}"></span></button>`,
        )
          .click(() => {
            if (this.check([this.modal.modal.find('#mel-publish-service')])) {
              ...
              rcmail.set_busy(true);
              rcmail.display_message(
                rcmail.gettext('adding', plugin_text),
                'loading',
              );
      ...
      for (let index = 0; index < itemToTest.length; index++) {
        ...
        if (valid) {
          element
            .parent()
            .append(
              `<span class="fieldNotOkay" style="color:red;">*${rcmail.gettext('error_field_required', plugin_text)}</span>`,
            );
          return false;
        }
      }
    ...
    confirm(caller, args) {
      switch (caller) {
        case 'createOrEditPublish':
          const service_datas = {
            name: $('#mel-publish-service')
              .find(`option[value="${$('#mel-publish-service').val()}"]`)
              .html(),
          ...
          this.modal.editTitle(
            rcmail.gettext('visualize_admin_news', plugin_text),
          );
          ...
          this.modal.footer.querry.html('');
          $(
            `<button class="mel-button white news-bottom-button-modal-l">${rcmail.gettext('back')} <span class="plus icon-mel-undo"></span></button>`,
          )
            .click(() => {
              this.createOrEditPublish(args.id.replace('news-', ''));
              let select = this.modal.modal.find('#mel-publish-service');
              if (
                select.find(`option[value="${service_datas.value}"]`).length ===
                0
              ) {
                select
                  .append(
                    `<option value="${service_datas.value}">${service_datas.name}</option>`,
                  )
                  .val(service_datas.value);
              }
            })
            .appendTo(this.modal.footer.querry);
          $(
            `<button class="mel-button news-bottom-button-modal-r">${rcmail.gettext('confirmation', plugin_text)} <span class="plus icon-mel-plus"></span></button>`,
          )
            .click(() => {
              if (
                args.trueId !== '' &&
                args.trueId !== null &&
                args.trueId !== undefined
              )
                news.id = args.trueId;
              ...
              rcmail.set_busy(true);
              rcmail.display_message(
                rcmail.gettext('publishing', plugin_text),
                'loading',
              );
        ...
        default:
          console.error(
            "###[confirm]Impossible de créer une fenêtre de confirmation pour cette action, elle n'éxiste pas !",
            caller,
          );
          break;
      }
    ...
    showStep2(datas, isEdit = false, isAdmin = false) {
      ...
      switch (datas.type) {
        case MelNews.type.intranet:
          if (!isEdit)
            this.modal.editTitle(
              rcmail.gettext('personal_add_intranet_title', plugin_text),
            );
          else
            this.modal.editTitle(
              rcmail.gettext('personal_edit_intranet_title', plugin_text),
            );
        ...
        case MelNews.type.twitter:
          if (!isEdit)
            this.modal.editTitle(
              rcmail.gettext('personal_add_twitter_title', plugin_text),
            );
          else
            this.modal.editTitle(
              rcmail.gettext('personal_edit_twitter_title', plugin_text),
            );

          html += this.createInput(
            rcmail.gettext('enter_twitter_account', plugin_text),
            'news-twitter-input',
            'text',
            `@${rcmail.gettext('enter_twitter_account_placeholder', plugin_text)}`,
            datas.id,
            'mel-twitter',
            'form-control input-mel required',
          );
          break;
        ...
        case MelNews.type.internet:
          ...
          else this.modal.editTitle('Modifier un flux RSS externe');

          html += this.createSelect(
            'Choisir le flux RSS à afficher',
            'news-internet-select',
      ...
      html += this.createCheckBoxChoices(
        rcmail.gettext('choose_block_format', plugin_text),
        'newsMelFormat',
        formats,
        datas.format,
        true,
      ...
      if (
        datas.type === MelNews.type.intranet &&
        isAdmin !== false &&
        atob(isAdmin) == 'true'
      ) {
        html = $(
          `<div style="text-align:right;"><button id="publishmode" class="mel-button" style="margin-top:0;margin-bottom:15px;float:">${rcmail.gettext('go_to_publisher_mode', plugin_text)}</button></div>`,
        );
      ...
      $(
        `<button class="mel-button white .news-bottom-button-modal-l">${isEdit ? rcmail.gettext('cancel') : rcmail.gettext('back')} <span class="plus icon-mel-undo"></span></button>`,
      )
        .click(() => {
          if (!isEdit) this.createOrEdit(datas.id);
          else this.modal.close();
        })
        .appendTo(this.modal.footer.querry);

      $(
        `<button class="mel-button news-bottom-button-modal-r">${!isEdit ? rcmail.gettext('add', plugin_text) : rcmail.gettext('edit', plugin_text)} <span class="plus icon-mel-${datas.id === '' ? 'plus' : 'pencil'}"></span></button>`,
      )
        .click(() => {
          let url; //= (datas.type === intranet ? $("#news-intranet-select") : $("#news-twitter-input")).val();

          if (datas.type === 'intranet')
            url = this.modal.modal.find('#news-intranet-select').val();
          ...
          if (url === '' || url === 'none') {
            rcmail.display_message(
              rcmail.gettext('error_invalid_value', plugin_text),
              'error',
            );
            return;
          }
          ...
          (this.isSettings() ? parent.rcmail : rcmail).set_busy(
            true,
            'loading',
          );
          ...
          if (isEdit) {
            this.post(
              ...
              {
                _url: url,
                _format: this.modal.modal.find('#mel-news-format-check-1')[0]
                  .checked
                  ? 'small'
                  : 'large',
                _last_url: datas.id,
              ...
              (response) => {
                ...
                if (response === 'denied') {
                  if (datas.type === 'twitter') {
                    this.modal.show();
                    (isSettings ? parent.rcmail : rcmail).display_message(
                      rcmail.gettext('error_invalid_account', plugin_text),
                      'error',
                    );
                  }
                  ...
                  this.modal.show();
                  (isSettings ? parent.rcmail : rcmail).display_message(
                    rcmail.gettext('error_maj', plugin_text),
                    'error',
                  );
                } else {
                  if (mode === modes.all) {
                    (isSettings ? parent.rcmail : rcmail).set_busy(
                      true,
                      'loading',
                    );
    ...
    filter() {
      ...
      const filters = {
        type: {
          ...
          array: [
            {
              value: 'intranet',
              text: rcmail.gettext('intranet_websites', plugin_text),
              id: 'mel-new-filter-1',
            } /*, {value:"rss", text:"Flux RSS", id:"mel-new-filter-3"}*/,
          ],
        },
        category: {
          ...
          array: [
            {
              value: 'news',
              text: rcmail.gettext('headlines', plugin_text),
              id: 'mel-new-filter-4',
            },
            /*{value:"defaults", text:"Sites par défauts", id:"mel-new-filter-5"},*/ {
              value: 'custom',
              text: rcmail.gettext('your_personal', plugin_text),
              id: 'mel-new-filter-6',
            },
          ],
        },
      ...
      this.modal.footer.querry.html('');
      $(
        `<button class="mel-button news-bottom-button-modal-r">${rcmail.gettext('Filtrer', plugin_text)} <span class="plus icofont-filter"></span></button>`,
      )
        .click(async () => {
    ...
    async updateFilter() {
      ...
      if (this.filters[undefined] !== undefined) delete this.filters[undefined];
    ...
    createInput(
      ...
      let html =
        title !== null
          ? `<label for="${id}" class="span-mel t1 first">${title}<span style="color:red">*</span> </label>`
          : '';
    ...
    createCheckBoxChoices(
      ...
      if (format) html += '<div class="row">';

      for (const key in values) {
        if (Object.hasOwnProperty.call(values, key)) {
          const element = values[key];

          if (format) html += `<div class="col-md-${12 / values.length}">`;
          ...
          html += this.createInput(
            ...
            ' custom-control-input required ' +
              (isRadio ? '' : 'form-check-input'),
            typeof value === 'string'
              ? value === element.value
                ? 'checked'
                : ''
              : value[element.id] === false
                ? ''
                : 'checked',
          );
  ...
  class MelNews {
    ...
    setup(id) {
      if (this.$news === undefined || this.$news === null) {
        try {
          ...
          if (this.$news.length === 0) this.$news = $(`[data-uid="${id}"]`);
        } catch (error) {
          try {
            if (this.$news === null) this.$news = $(`[data-uid="${id}"]`);
          } catch (error) {}
        }
      }
    ...
    setup_vignette() {
      ...
      if (this.type === MelNews.type.twitter) return this;
      ...
      if (
        this.$news.find('.vignette-arrows').length === 0 &&
        (rcmail.env.news_vignette_all_news_datas[this.type] !== undefined ||
          rcmail.env.news_vignette_all_news_datas[this.id] !== undefined)
      ) {
        ...
        $left.click((e) => {
          ...
          else news_datas = rcmail.env.news_vignette_all_news_datas[this.id];
          ...
          try {
            if (this.current - 1 === -1) {
              tmp = this.original;
              this.current = -1;
            } else tmp = news_datas[--this.current];
          } catch (error) {
            tmp = undefined;
          }
          ...
          if (
            this.type === MelNews.type.intranet &&
            this.$news.data('copy') ===
              ($(tmp).children().data('copy') === undefined
                ? $(tmp).data('copy')
                : $(tmp).children().data('copy'))
          ) {
            reclick = true;
          }
          ...
          this.$news
            .data('copy', $(tmp).children().data('copy'))
            .find('.square-contents')
            .append($(tmp).find('.square-contents').html());

          if (this.current === -1)
            $(e.currentTarget)
              .addClass('disabled')
              .attr('disabled', 'disabled');
          else if (reclick) this.$left.click();
        });
        ...
        $right.click((e) => {
          ...
          if (news_datas[this.current + 1] === undefined)
            $(e.currentTarget)
              .addClass('disabled')
              .attr('disabled', 'disabled');
          else if (reclick) this.$right.click();
        });

        this.$news.find('.square-contents').prepend($html);

        this.$right = this.$news
          .find('.square-contents')
          .find('.icon-mel-arrow-right')
          .parent();
        this.$left = this.$news
          .find('.square-contents')
          .find('.icon-mel-arrow-left')
  ...
  class MelCustomNews extends MelNews {
    ...
    static fromServer(serverDatas, original = null) {
      ...
      created.format =
        original !== null && serverDatas.size === 'same'
          ? original.format
          : serverDatas.size;
      created.link = serverDatas.datas.link;
      created.site = serverDatas.service;
      created.type =
        original !== null && serverDatas.source === 'same'
          ? original.type
          : serverDatas.source;
      created.$news = $(
        created.loadHtml(rcmail.env.news_skeleton, serverDatas.datas),
      );
    ...
    setup(id) {
      ...
      if (!this.date.isValid()) this.date = moment();
    ...
    loadHtml(skeleton, content, optional_style = 'style="margin-bottom:15px"') {
      return skeleton
        ...
        .replaceAll(
          ...
          `<div style="position: absolute;bottom: 30px;" class="headlines-source"><p>Source : ${strUcFirst(this.type)}</p><p class='p-buttons'>
        ...
        .replaceAll(
          '<date/>',
          `Publié le ${this.tradDate(this.date.format('dddd DD MMMM YYYY'))}`,
        );
    }
    ...
    _refresh_mode_all(datas) {
      let oldNews = Enumerable.from(MelCustomNews.allCustomNews).where(
        (x) => x.id !== this.id,
      );
      let newNews = [];
      ...
      oldNews.forEach((x, i) => {
        ...
        if (x.$news.length > 1) x.$news = $(x.$news[0]);
      });
      ...
      for (const key in MelCustomNews.allCustomNews) {
        if (Object.hasOwnProperty.call(MelCustomNews.allCustomNews, key)) {
          ...
          MelCustomNews.allCustomNews[key].$news =
            MelCustomNews.allCustomNews[key].$news.appendTo($col);

          if (MelCustomNews.allCustomNews[key].type === MelNews.type.twitter) {
            MelCustomNews.allCustomNews[key].$news
              .find('.headlines-contents')
              .html(
                `<a class="twitter-timeline" href="https://twitter.com/${MelCustomNews.allCustomNews[key].id}?ref_src=twsrc%5Etfw"></a>`,
              );

            if (!started) started = true;
          } else if (
            MelCustomNews.allCustomNews[key].type !== MelNews.type.twitter &&
            MelCustomNews.allCustomNews[key].$news.data('copy') !== ''
          ) {
            MelCustomNews.allCustomNews[key].$news
              .find('.square-contents')
              .children()
              .each((i, e) => {
                if ($(e).hasClass('vignette-arrows')) return;
    ...
    static CreateAndSetup(initialRefresh = false) {
      ...
      $('#i-custom-news .mel-news').each((i, e) => {
        ...
        if ($(e).hasClass('rss')) array.push(new MelCustomNews(uid));
        else {
          array.push(new MelNews(uid).setNews($(e)).setup(uid));
        }

        if (
          array[array.length - 1].type !== MelNews.type.twitter &&
          array[array.length - 1].$news.data('copy') !== ''
        ) {
          array[array.length - 1].$news
            .find('.square-contents')
            .children()
            .each((i, e) => {
              if ($(e).hasClass('vignette-arrows')) return;
              ...
              $(e)
                .css('cursor', 'pointer')
                .attr('title', rcmail.gettext('open_in_new_tab', plugin_text));
            });
        }
      });
    ...
    static async Refresh(initialRefresh = false) {
      ...
      const button_initial_class = $button.hasClass('icon-mel-chevron-down')
        ? 'icon-mel-chevron-down'
        : 'icon-mel-chevron-up';
      $button
        ...
        for (const key in MelCustomNews.allCustomNews) {
          if (Object.hasOwnProperty.call(MelCustomNews.allCustomNews, key)) {
            ...
            if (!array_id.includes(element.id)) {
              if (initialRefresh === true && element.site !== '') continue;

              if (element.type === MelNews.type.twitter) continue;
      ...
      $('#seemoreorless .plus')
        .addClass(button_initial_class)
        .removeClass('spinner-grow')
        .removeClass('spinner-grow-sm')
    ...
    static async Reorder() {
      ...
      $('#i-custom-news-temp').html(
        '<center><div class="spinner-grow"><span class="sr-only">Loading....</span></div></center>',
      );

      let initialclass = $('#seemoreorless .plus').hasClass(
        'icon-mel-chevron-down',
      )
        ? 'icon-mel-chevron-down'
        : 'icon-mel-chevron-up';
      ...
      for (const key in news) {
        if (Object.hasOwnProperty.call(news, key)) {
          ...
          if (element.format === 'small') {
            $col = $(
              `<div class="col-3" style="display:${element.$news.parent().css('display')}"></div>`,
            );
          } else {
            $col = $(
              `<div class="col-6" style="display:${element.$news.parent().css('display')}"></div>`,
            );
          }
          ...
          if (element.type === MelNews.type.twitter) {
            element.$news
              .find('.headlines-contents')
              .html(
                `<a class="twitter-timeline" href="https://twitter.com/${element.id}?ref_src=twsrc%5Etfw"></a>`,
              );
        ...
        if (Object.hasOwnProperty.call(filters, key)) {
          ...
          for (const name in filter) {
            if (Object.hasOwnProperty.call(filter, name)) {
              ...
              const $element =
                _element.length === 0
                  ? [{ checked: NewsPopup.fabric().filters[filter[name]] }]
                  : _element;
                ...
                if (key === 'types') {
                  ...
                  switch (name) {
                    case 'headlines': //defaults/customs
                      ...
                      enum_news = enum_news.where(
                        (x) => !x.type.includes('ou='),
                      );
                      break;
                    ...
                    case 'customs':
                      promises.push(
                        new Promise((a, b) => {
                          enum_news
                            .where(
                              (x) =>
                                x.type === 'twitter' || x.type === 'intranet',
                            )
                            .forEach((x) => {
                              x.$news.parent().css('display', 'none');
                            });
                      ...
                      enum_news = enum_news.where(
                        (x) => x.type !== 'twitter' && x.type !== 'intranet',
                      );
                      break;
  ...
  class MelPublishNew extends MelNews {
    ...
    startup(id) {
      if (id !== null) {
        ...
        if (id.includes('@')) this.id = id.split('@')[0];
      }
    ...
    toPostDatas(ignoreId = null) {
      ...
      if (ignoreId !== this.id) item._uid = this.id;
  ...
  function init_news() {
      ...
      (x) => {
        if (typeof x === 'string') mel_metapage.Functions.copy(x);
          ...
          mel_metapage.Functions.copy(
            x.parent().parent().parent().data('copy'),
          );
      },
      true,
    );
    ...
    rcmail.register_command(
      ...
      (x) => {
        ...
        NewsPopup.fabric()
          .drawChoice(
            rcmail.gettext('draw_choice_title', plugin_text),
            {
            ...
            },
            {
              ...
              action: () => {
                ...
                if (
                  mode === modes.all &&
                  $(x).parent().parent().parent().data('type') ===
                    MelNews.type.intranet
                ) {
                  text = rcmail.gettext(
                    'delete_flux_warning_all_intra',
                    plugin_text,
                  );
                } else
                  text = rcmail.gettext('delete_flux_warning', plugin_text);
                ...
                if (confirm(text)) {
                  ...
                  mel_metapage.Functions.post(
                    mel_metapage.Functions.url('news', 'delete_custom'),
                    {
                      _url: new MelCustomNews(
                        $(x).parent().parent().parent().data('uid'),
                      ).id,
                    },
                    (datas) => {
                      window.location.reload();
    ...
    rcmail.register_command(
      ...
      (x) => {
        if (x._event !== undefined) {
          let $parent = $(
            x._event.originalTarget === undefined
              ? x._event.target
              : x._event.originalTarget,
          );
          let it = 0;
          while (it < 3) {
            if ($parent.hasClass('vignette-arrows')) return;
            else {
        ...
        if (
          element.data('site') !== MelNews.type.twitter &&
          element.data('copy') !== ''
        ) {
    ...
    rcmail.register_command(
      'news.keydown',
      (x) => {
        if (x._event.keyCode === 13)
          rcmail.command('news.click', {
            _event: undefined,
            element: x.element,
          });
      },
    ...
    if (
      storage === null ||
      moment(storage).startOf('day').format('DD/MM/YYYY') !==
      ...
      rcmail.set_busy(true);
      rcmail.display_message(
        rcmail.gettext('update_news', plugin_text),
        'loading',
      );
    }
    ...
    MelCustomNews.CreateAndSetup(initial).then(() => {
      if (initial === false) {
        ...
        rcmail.clear_messages();
        rcmail.display_message(
          rcmail.gettext('new_successfuly_updated', plugin_text),
          'confirmation',
        );
      }

      if (
        Enumerable.from(MelCustomNews.allCustomNews).any(
          (x) => x.type === MelNews.type.twitter,
        )
      ) {
        $('script[src="https://platform.twitter.com/widgets.js"]').remove();
        setTwitterSrc();
      ...
      if (
        Enumerable.from(MelCustomNews.allCustomNews)
          .select((x) => (x.format === 'small' ? 3 : 6))
          .sum() <
        12 * nbRows
      )
    ...
    $('#seemoreorless').click(() => {
      if (rcmail.env.news_more_showed === 'loading') return;

      if (rcmail.env.news_more_showed !== true) {
        let $querry = $('#seemoreorless .plus')
          .removeClass('icon-mel-chevron-down')
          .addClass('spinner-grow spinner-grow-sm');
        ...
        mel_metapage.Functions.get(
          mel_metapage.Functions.url('news', 'show_all_custom_news'),
          {},
          (datas) => {
            let $parent = $('#i-custom-news');
                ...
                if (mode === modes.vignette) {
                  ...
                  for (const key in MelCustomNews.allCustomNews) {
                    if (
                      ...
                      if (
                        MelCustomNews.allCustomNews[key].type === 'twitter' &&
                        !started
                      )
                        started = true;
                    }
        ...
        $('#seemoreorless .plus')
          .removeClass('icon-mel-chevron-up')
    ...
    $('#news-select-order').on('change', () => {
      ...
      Promise.all([
        mel_metapage.Functions.post(
          mel_metapage.Functions.url('news', 'update_sort'),
          {
        ...
        MelCustomNews.Reorder(),
      ]).then(() => {
        if (!ok)
          rcmail.display_message(
            rcmail.gettext('error_reorder', plugin_text),
            'error',
          );
      });
    });
    ...
    rcmail.addEventListener('mel_metapage_refresh', async () => {
      ...
      mel_metapage.Functions.get(
        ...
        (success) => {
          ...
          if (
            rcmail.env.news_service_for_publish !== null &&
            rcmail.env.news_service_for_publish !== undefined &&
            Enumerable.from(rcmail.env.news_service_for_publish).any()
          ) {
      ...
      for (let index = 0; index < MelCustomNews.allCustomNews.length; index++) {
        if (
          MelCustomNews.allCustomNews[index].type !== MelNews.type.twitter &&
          MelCustomNews.allCustomNews[index].$news.data('copy') !== ''
        ) {
          MelCustomNews.allCustomNews[index].$news
            .find('.square-contents')
            .children()
            .each((i, e) => {
              if ($(e).hasClass('vignette-arrows')) return;
