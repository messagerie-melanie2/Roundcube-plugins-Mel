...
if (window.rcmail) {
	rcmail.addEventListener('init', function(evt) {
			if (rcmail.env.task == 'settings'
				...
				rcmail.addEventListener('responseafterplugin.mel_doubleauth-removeuser', function(evt) {
					$('#2FA_activate_button').removeAttr('disabled');
					$('.settingsbox table tr.added').remove();
				...
				rcmail.addEventListener('responseafterplugin.mel_doubleauth-adduser', function(evt) {
						...
						$('table tr:last')
								.before(
										"<tr class=\"added\"><td width='20px'><span class='cercle'>2</span></td><td colspan='2'>"
												+ rcmail.gettext('action_2',
														'mel_doubleauth')
												+ '<div id="img_qr_code" style="display: visible; "></div></td></tr>');
						// add qr-code before msg_infor
						var url_qr_code_values = 'otpauth://totp/'
								+ $('#prefs-title').html().split(/ - /)[1]
								+ '?secret=' + $('#2FA_secret').get(0).value
								+ '&issuer=M2Web';
						$('table tr:last')
								.before(
						...
						$('table tr:last')
								.before(
										"<tr class=\"added\"><td width='20px'><span class='cercle'>4</span></td><td colspan='2'>"
												+ rcmail.gettext('action_4',
														'mel_doubleauth')
												+ '&nbsp;&nbsp;<input type="text" id="2FA_code_to_check" maxlength="10" onkeypress="if (event.keyCode == 13) return false;">&nbsp;&nbsp;<input type="button" class="button mainaction" id="2FA_check_code" value="'
												+ rcmail.gettext('check_code',
														'mel_doubleauth') + '"></td></tr>');
	
						$('table tr:last').before(
								"<tr class=\"added\"><td width='20px'><span class='cercle'>5</span></td><td colspan='2'>"
										+ rcmail.gettext('action_5', 'mel_doubleauth')
										+ '</td></tr>');
						$('table tr:last')
								.before(
										'<tr class=\"added\"><td colspan="3"><p><input type="button" class="button mainaction 2FA_save" id="rcmbtn120" onclick="return rcmail.command(\'plugin.mel_doubleauth-save\',\'\',this,event)" value="Enregistrer" disabled="disabled"></p></td></tr>');
	
						// ajax
						$('#2FA_check_code')
								.click(
										function() {
											url = "./?_action=plugin.mel_doubleauth-checkcode&code="
													+ $('#2FA_code_to_check').val();
											$.post(url, function(data) {
												alert(data);
												if (data == rcmail.gettext('code_ok',
														'mel_doubleauth'))
			...
			function setup2FAfields() {
				...
				$('.2FA_save').attr('disabled', 'disabled').attr('title',
			...
			$('#2FA_show_recovery_codes').click(
					function() {
						if ($("[name^='2FA_recovery_codes']")[0].type == 'text') {
							$("[name^='2FA_recovery_codes']").each(function() {
								$(this).get(0).type = 'password';
							});
							$('#2FA_show_recovery_codes').get(0).value = rcmail
									.gettext('show_recovery_codes', 'mel_doubleauth');
						} else {
							$("[name^='2FA_recovery_codes']").each(function() {
								$(this).get(0).type = 'text';
							});
							$('#2FA_show_recovery_codes').get(0).value = rcmail
									.gettext('hide_recovery_codes', 'mel_doubleauth');
						}
					});
			...
			click2FA_change_qr_code = function() {
				if ($('#2FA_qr_code').is(':visible')) {
					$('#2FA_qr_code').slideUp();
					$(this).get(0).value = rcmail.gettext('show_qr_code',
							'mel_doubleauth');
				} else {
					$('#2FA_qr_code').slideDown();
					$(this).get(0).value = rcmail.gettext('hide_qr_code',
							'mel_doubleauth');
				}
			}
			...
			rcmail.register_command('plugin.mel_doubleauth', function() {
