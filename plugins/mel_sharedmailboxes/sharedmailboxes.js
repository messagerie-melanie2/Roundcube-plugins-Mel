  ...
  rcmail.addEventListener('init', function (evt) {
    if (rcmail.treelist) {
      rcmail.treelist.addEventListener('collapse', function (node) {
        if (
          rcmail.env.folders_display != 'unified' &&
          node.classes.includes('boite')
        ) {
          folder_collapse(node.id);
        } else if (
          rcmail.env.folders_display == 'unified' &&
          node.classes.includes('virtual') &&
          (node.classes.includes('inbox') ||
            node.classes.includes('drafts') ||
            node.classes.includes('models') ||
            node.classes.includes('sent') ||
            node.classes.includes('junk') ||
            node.classes.includes('trash'))
        ) {
    ...
    if (
      ...
      if (rcmail.env.folders_display == 'unified') {
        $('#mailboxlist > .mailbox.virtual').each(function () {
          if (
            !$(this).hasClass('boite') &&
            $(this).find('> div.treetoggle').hasClass('collapsed')
          ) {
            var node_id = rcmail.html_identifier_decode(
              $(this).attr('id').replace(/rcmli/, ''),
            );
...
function set_title(mycount) {
  ...
  else if (mycount) new_title = '(' + mycount + ') ' + doc_title;
  else new_title = doc_title.replace(reg, '');
...
function folder_collapse_unified(node_id) {
  ...
  $(li)
    .find(' > ul > .mailbox')
    .each(function () {
      const child_id = rcmail.html_identifier_decode(
        $(this).attr('id').replace(/rcmli/, ''),
      );
...
function folder_collapse(node_id) {
  ...
  const child_id = rcmail.html_identifier_decode(
    li.querySelector('.mailbox.inbox').id.replace(/rcmli/, ''),
  );
