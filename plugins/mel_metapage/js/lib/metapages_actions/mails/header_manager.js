...
export class UIMailHeaderManager extends MailModule {
  ...
  main() {
    ...
    const header_actions = MelEnumerable.from(this.select(lc_selector))
      .where((x) => $(x).css('display') !== 'none')
      .where((x) => {
        return MelEnumerable.from(x.classList).any((a) =>
          ['tb_noclass', 'move', 'delete'].includes(a),
        );
      })
      .reverse();
  ...
  set_listeners() {
    ...
    for (const iterator of listeners) {
      this.rcmail().addEventListener(
        iterator,
        this[`on_${iterator.replaceAll('.', '_').replaceAll('-', '_')}`].bind(
          this,
        ),
      );
    }
    ...
    rcmail.message_list.addEventListener('select', () => {
      ...
      if (!this.has_previsu() && this.has_mail_selected()) {
        if (!$list.hasClass('elements-enabled'))
          $list
            .addClass('elements-enabled')
            .find('li a')
            .removeClass('disabled')
            .removeAttr('aria-disabled')
            .removeAttr('disabled');
      } else if ($list.hasClass('elements-enabled'))
        $list
          .removeClass('elements-enabled')
          .find('li a')
          .addClass('disabled')
  ...
  on_ui_open_mail_visu() {
    if (this.get_list().css('display') !== 'none') this.get_list().hide();
  }
