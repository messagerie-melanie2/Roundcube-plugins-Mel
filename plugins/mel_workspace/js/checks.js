...
function check_workspace_integrity() {
  ...
  const always_enabled_services = [services.links];
  ...
  for (const key in workspace_services) {
    if (Object.hasOwnProperty.call(workspace_services, key)) {
      ...
      if (element || always_enabled_services.includes(key)) {
        ...
        if (
          toolbar_item &&
          $(`.wsp-toolbar.melw-wsp .${toolbar_item}`).length > 0
        ) {
          items[key] = $(`.wsp-toolbar.melw-wsp .${toolbar_item}`)
            .addClass('disabled')
        ...
        ajax_call = mel_metapage.Functions.post(
          ...
          (datas) => {
            ...
            if (datas.state === states.invalid) {
              //Si le service a été supprimer
              const isCustomState = !!datas.service_state?.state;
              const text =
                isCustomState && !!datas.service_state.text
                  ? datas.service_state.text
                  : `Nous ne parvenons pas à trouver des données pour le service "${rcmail.gettext(datas.service, 'mel_workspace')}", le service a dû être supprimé par un des administrateurs, création du service en cours....`;
              console.error(`###[checks]${text}`, datas);
              rcmail.display_message(text, 'error');

              let child_call = mel_metapage.Functions.post(
                //On créer le service
                mel_metapage.Functions.url('workspace', 'create_service_async'),
                {
                  _id: uid,
                  _service: datas.service,
                  _state: isCustomState
                    ? datas.service_state.custom_args
                    : datas.service_state,
                },
                (creation_datas) => {
                  ...
                  if (creation_datas !== 'false') {
                    ...
                    if (creation_datas.env) {
                      for (const key in creation_datas.env) {
                        if (
                          Object.hasOwnProperty.call(creation_datas.env, key)
                        ) {
                    ...
                    if ((creation_datas.update_button?.length ?? 0) > 0) {
                      for (const iterator of creation_datas.update_button) {
                        let $querry = iterator.is_class
                          ? $(`.wsp-toolbar.melw-wsp .${iterator.id}`)
                          : $(`#${iterator.id}`);

                        if (iterator.datas) {
                          for (const key in iterator.datas) {
                            if (
                              Object.hasOwnProperty.call(iterator.datas, key)
                            ) {
                    ...
                    if ((creation_datas.triggers?.length ?? 0) > 0) {
                      for (const iterator of creation_datas.triggers) {
                        if (typeof iterator === 'string')
                          rcmail.triggerEvent(iterator);
                        else {
                          if ((iterator?.is_top ?? false) === true)
                            top.rcmail.triggerEvent(iterator.trigger);
                    ...
                    if (isCustomState && !!datas.service_state.ok_text)
                      displayedText = datas.service_state.ok_text;
                    else
                      displayedText = `Le service ${rcmail.gettext(datas.service, 'mel_workspace')} a été créé avec succès !`;

                    rcmail.display_message(displayedText, 'confirmation');

                    if (always_enabled_services.includes(datas.service)) {
                      if (
                        confirm(
                          'Le service a été créé avec succès, voulez-vous rafraichir la page pour voir les changements ?',
                        )
                      ) {
                        const url =
                  ...
                  if (items[datas.service])
                    set_toolbar_item_valid(
                      items[datas.service].find('span').first(),
                    );
                },
              );
