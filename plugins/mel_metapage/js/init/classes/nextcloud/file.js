...
function Nextcloud_File(xml) {
  ...
  for (let index = 0; index < this.href.length; index++) {
    const element = this.href[index];
    if (element === '/') {
      if (index === this.href.length - 1) break;
      lastText = text;
      text = '';
    } else text += element;
  }
  ...
  if (this.folder_parent === 'files') this.folder_parent = null;
  text = lastText = null;
  //console.log(xml.getElementsByTagName("d:propstat"));
  let prop = Enumerable.from(xml.getElementsByTagName('d:propstat'))
    .select((x) => x.getElementsByTagName('d:status')[0])
    .where((x) => x.innerHTML.includes('OK'))
    .firstOrDefault(null);
  this.is_valid_file = prop !== null;
  if (!this.is_valid_file) return;
  prop = prop.parentElement.getElementsByTagName('d:prop')[0];
  try {
    this.lastModified = new Date(
      prop.getElementsByTagName('d:getlastmodified')[0].innerHTML,
    );
  } catch (error) {
  ...
  try {
    this.type =
      prop.getElementsByTagName('d:resourcetype')[0].children.length !== 0
        ? mel_metapage.Symbols.nextcloud.folder
        : mel_metapage.Symbols.nextcloud.file;
  } catch (error) {
    this.type = undefined;
  }
