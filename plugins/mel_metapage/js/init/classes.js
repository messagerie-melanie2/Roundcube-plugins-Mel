...
class ArianePopUp {
  ...
  _show() {
    ...
    if (
      ...
      this.button.button
        .find('.icon-mel-message')
        .removeClass('icon-mel-message')
        .addClass('icon-mel-close');
    else this.button.button.css('display', 'none');
...
ArianePopUp.set_width = function (node) {
  let set_width = function (width) {
    let max = 300;
    if (width > window.innerWidth / 2.0) width = window.innerWidth / 2.0;
    node.css({
      width: width,
...
class ArianeFrame {
  constructor() {
    ...
    if ($('.discussion-frame').length > 0) {
      ...
      new Promise(async (a, b) => {
        while (
          !rcmail.env.frame_created ||
          $('.discussion-frame').length ==
            0 /* || $(".discussion-frame").contents().find("#rocket_chat_frame").length == 0*/
        ) {
          await delay(500);
        }
        this.popUp = $('.discussion-frame');
  ...
  is_anchor() {
    return (
      this.card.card.card.find('.card-anchor').text() === ArianeFrame.unanchor
    );
  }
  ...
  enable() {
    ...
    window.onresize = function () {
      if (
        ...
        mel_metapage.PopUp.ariane.ariane.popUp.css(
          'height',
          height - 60 - 45 + 'px',
        );
        mel_metapage.PopUp.ariane.ariane.card.card.card.css('height', '100%');
        mel_metapage.PopUp.ariane.ariane.card.body.card.css(
          'height',
          height + 'px',
        );
        new Promise(async (a, b) => {
          ...
          if (window.location.href.includes('&_action=chat')) return;
...
class ArianeCard {
  ...
  enable() {
    if (!this.is_enabled())
      this.card
        ...
        .addClass(this.enabled_string(false));
  }
  ...
  disable() {
    if (this.is_enabled())
      this.card
        .css('display', '')
        .removeClass(this.enabled_string(false))
  ...
  hide_button = function () {
    ...
    if (
      ...
      this.button
        ...
        .css('display', 'initial');
    else this.button.css('display', 'none');
  };
...
class MetapageFrames {
  ...
  triggerEvent(key, ...args) {
    if (this._events[key] === undefined || this._break) return;
    else {
      let result = null;

      for (let index = 0; index < this._events[key].length; index++) {
        const element =
          typeof this._events[key][index] !== 'function' &&
          this._events[key][index]?.ignore_context
            ? eval(this._events[key][index].ev + '')
            : this._events[key][index];
        try {
          if (index === 0) result = element(...args);
          else {
            if (result !== null && result !== undefined)
              result = element(...[...args, result]);
            else result = element(...args);
          }
        } catch (error) {
          console.error(error);
        }
...
class Roundcube_Mel_Color {
  ...
  setup() {
    ...
    if (!this.color) {
      if ($('html').hasClass('dark-mode'))
        this.color = Roundcube_Mel_Color.dark;
  ...
  setColor(color) {
    if (color === Roundcube_Mel_Color.light && this.isDarkMode())
      this.switch_color();
    else if (color === Roundcube_Mel_Color.dark && this.isLightMode())
      this.switch_color();
    else if (
      color !== Roundcube_Mel_Color.dark &&
      color !== Roundcube_Mel_Color.light
    ) {
    ...
    if (this.config.localcache) {
      ...
      try {
        ...
        if (Object.keys(self.store).length <= 1) {
          console.error(
            `###[MelDataStore]Impossible d'ajouter ${id} au stockage local`,
            self.getSize(),
            e,
            self,
            val,
          ...
          self._removeOldestValue().set(id, val);
        }
      }
    } else {
      this.store[id] = val;
