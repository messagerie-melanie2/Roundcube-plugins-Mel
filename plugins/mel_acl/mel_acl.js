  ...
  rcmail.addEventListener('init', function () {
    ...
    if (rcmail.gui_objects.acltable) {
      ...
      if (rcmail.env.acl_users_source) {
        inst = rcmail.is_framed() ? parent.rcmail : rcmail;
        if (rcmail.env.type.indexOf('group') !== -1) {
          inst.init_address_input_events($('#acluser'), {
        ...
        inst.set_env({
          autocomplete_max: rcmail.env.autocomplete_max,
          autocomplete_min_length: rcmail.env.autocomplete_min_length,
        });
    ...
    if (rcmail.env.acl_advanced) $('#acl-switch').addClass('selected');
  });
}
...
rcube_webmail.prototype.acl_on_close = function () {
  ...
  if ($querry.length === 0) {
    let i = 0;
    while ($querry.length === 0 && context !== top) {
      if (i++ >= 10) break;
    ...
    if ($querry.length !== 0) $querry.css('display', 'none');
...
rcube_webmail.prototype.acl_delete = function () {
  ...
  if (
    users &&
    users.length &&
    confirm(this.get_label('mel_acl.deleteconfirm'))
  ) {
    this.http_post(
      'settings/plugin.acl',
      {
...
rcube_webmail.prototype.acl_save = function () {
  var user = $('#acluser', this.acl_form).val(),
    rights = '',
    type,
    data,
    objects = '';
  ...
  if (this.env.type && this.env.type === 'm2mailbox') {
    ...
    $(
      this.env.acl_advanced
        ? '#advancedrights :checkbox'
        : '#simplerights :checkbox',
      this.acl_form,
    ).map(function () {
      if (this.checked) {
        if (objects !== '') objects += ',';
  ...
  this.http_post(
    'settings/plugin.acl',
    data,
    this.set_busy(true, 'mel_acl.saving'),
  );
};
...
rcube_webmail.prototype.acl_update = function (o) {
  // delete old row
  if (o.old) this.acl_remove_row(o.old);
  // make sure the same ID doesn't exist
  else if (this.env.acl[o.id]) this.acl_remove_row(o.id);
...
rcube_webmail.prototype.acl_mode_switch = function (/*elem*/) {
  ...
  this.http_request(
    ...
    '_act=list' +
      '&_mode=' +
      (this.env.acl_advanced ? 'advanced' : 'simple') +
      '&_mbox=' +
      urlencode(this.env.mailbox),
...
rcube_webmail.prototype.acl_list_keypress = function (list) {
  if (list.key_pressed === list.ENTER_KEY) this.command('acl-edit');
  else if (
...
rcube_webmail.prototype.acl_get_usernames = function () {
  ...
  for (n = 0, len = selection.length; n < len; n++) {
    if (
      ...
      $.inArray(selection[n], this.env.acl_specials) >= 0
    ) {
...
rcube_webmail.prototype.acl_add_row = function (o, sel) {
  ...
  for (n in this.env.acl) {
    if (this.env.acl[n]) {
      if (
        this.env.acl_specials.length &&
        $.inArray(n, this.env.acl_specials) >= 0
      )
        spec.push(n);
      else ids.push(n);
    }
  ...
  // find current id
  for (n = 0, len = ids.length; n < len; n++) if (ids[n] === id) break;
  ...
  if (n && n < len) {
    ...
    list.init_row(row);
    list.rowcount++;
  } else list.insert_row(row);

  if (sel) list.select_row(o.id);
};
...
rcube_webmail.prototype.acl_init_form = function (id) {
  ...
  if (id && (row = this.acl_list.rows[id])) {
    ...
    li_elements.map(function () {
      val = this.value;
      td = $('td.' + this.id, row);
      if (td.length && td.hasClass('enabled')) this.checked = true;
    });
  ...
  for (const key in this.env.acl_objects[val]) {
    $('input[value="' + this.env.acl_objects[val][key] + '"]').prop(
      'checked',
      true,
    );
  }
  ...
  buttons[this.gettext('cancel')] = function (/*e*/) {
    me.command('acl-cancel');
    rcmail.triggerEvent('acl.onclose');
  };

  let popup_wrapper = $('<div style="width:480px; min-height:280px"></div>');
  ...
  else $('input:checked', type_list).focus();
};
...
rcube_webmail.prototype.acl_class = function (acl1, acl2) {
  ...
  if (found === len) return 'enabled';
  else if (found) return 'partial';
