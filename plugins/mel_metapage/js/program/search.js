$(document).ready(() => {
  ...
  class searchBlock {
    ...
    create() {
      this.$block = $(`<div data-type="${this.type}"></div>`).html(
        this.raw.html,
      );
      return this;
    }
  ...
  class search {
    ...
    research(word, config) {
      ...
      this.settings.$globaSearchInput
        .val(word)
        .addClass('disabled')
          ...
          success: async function (data) {
            try {
              ...
              if (data.datas !== undefined && data.datas.length > 0) {
                ...
                if (
                  !$('.body .loading .spinner-grow').hasClass('text-danger')
                ) {
    ...
    order() {
      ...
      if (this.settings.selects.$order.val() === 'asc')
        order = _enum.orderBy((x) =>
          moment(
            Enumerable.from(x.value.raw)
              .where((w) => w.key === 'datas')
              .select((s) => s.value.date)
              .toArray()[0] || moment.max(),
          ),
        );
      else
        order = _enum.orderByDescending((x) =>
          moment(
            Enumerable.from(x.value.raw)
              .where((w) => w.key === 'datas')
              .select((s) => s.value.date)
              .toArray()[0] || moment().subtract(moment().year(), 'y'),
          ),
        );

      const ordered = order.toJsonDictionnary(
        (x) => x.key,
        (x) => x.value,
      );
    ...
    _filter_action(
      ...
      if (filter === 'all') {
        ...
        if (isMail && filter_bal !== 'all') {
          //Recherche sur la bali ou les balp
          if (filter_bal === 'bali') {
            //Recherche sur la bali
            if (filter_bali_folder === 'all')
              showed = this._filter_action(
                element,
                key,
                `${rcmail.gettext('mail', 'mel_metapage')}/INBOX`,
                'all',
                'all',
                false,
                true,
              ...
              //Recherche sur une dossier de la bali
              if (
                key.includes(
                  `${rcmail.gettext('mail', 'mel_metapage')}/INBOX`,
                ) &&
                element.raw.datas.folder === filter_bali_folder
              ) {
                element.$block.css('display', '');
                showed = true;
          ...
          //Recherche sur une balp
            ...
            showed = this._filter_action(
              element,
              key,
              `${rcmail.gettext('mail', 'mel_metapage')}/Boite partag&AOk-e/${filter_bal.split('.-.')[1]}`,
              'all',
              'all',
              false,
              true,
        ...
        else if (include ? !key.includes(filter) : key !== filter)
          element.$block.css('display', 'none');
        else {
          element.$block.css('display', '');
          showed = true;
        }
    ...
    openParams() {
      const paramUrl = mel_metapage.Functions.url('settings', 'edit-prefs', {
        _section: 'globalsearch',
        _framed: 1,
      });
      ...
      let $html = $(
        `<iframe style='width:100%;min-height:300px' src=${paramUrl}></iframe>`,
      ).on('load', (e) => {
      ...
      $html = $(
        "<div><center id=search-param-loading><span class='spinner-grow'></span></center></div>",
      ).append($html);

      const config = new GlobalModalConfig(
        rcmail.gettext('search_settings', 'mel_metapage'),
        'default',
        $html,
      );
      ...
      modal.footer.buttons.save.click(() => {
        ...
        modal.contents
          .find('iframe')[0]
          .contentWindow.$('.formbuttons button')
          .click();
      });
      modal.show();
  ...
  if (
    Enumerable.from(rcmail.env.mm_search_config)
      .where((x) => x.includes('search_mail'))
      .any()
  ) {
    const base = Enumerable.from(rcmail.env.mm_search_config)
      .where((x) => x.includes('search_mail'))
      .first();
    for (const key in rcmail.env.shared_mailboxes) {
      if (Object.hasOwnProperty.call(rcmail.env.shared_mailboxes, key)) {
        ...
        rcmail.env.mm_search_config.push(
          `${base}&_mbox=${encodeURIComponent(`Boite partag&AOk-e/${key.split('.-.')[1]}`)}`,
        );
      }
    }
  }
  ...
  search.search = new search(
    rcmail.env.word_searched,
    rcmail.env.mm_search_config,
  );
  ...
  search.search.settings.$searchInput.on('change', () => {
    rcmail.command('mel.search', {
      word: search.search.settings.$searchInput.val(),
    });
  });
