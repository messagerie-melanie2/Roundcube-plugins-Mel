...
$(document).on(
  'submit',
  '#mainscreen-annuaire #quicksearchbar form',
  function (e) {
    e.preventDefault();
    var params = {
  ...
  $('#annuaire-list li.object').each(function () {
    ...
    if (search.length > 1) {
      if (obj.text().toLowerCase().indexOf(search) >= 0) {
        obj.html(obj.text());
        obj.html(
          obj.text().replace(new RegExp(search, 'gi'), '<strong>$&</strong>'),
        );
      } else {
        obj.html(obj.text());
        $(this).hide();
...
$(document).on('click', '#searchmenulink', function () {
  $('.contactslist .listsearchbox form').trigger('submit');
});
...
window.rcmail &&
  rcmail.addEventListener('init', function (evt) {
    ...
    if (rcmail.env.action == 'plugin.annuaire') {
      ...
      $('#directorylist li a[rel=' + rcmail.env.annuaire_source + ']')
        .parent()
      ...
      $('#annuaireselector').on('change', () => {
        ...
        if (window.annuaireSelector !== 'all') {
          $('#quicksearchbar .button.options').addClass(
            'annuaire-filter-selected',
          );
        } else {
          $('#quicksearchbar .button.options').removeClass(
            'annuaire-filter-selected',
          );
        }
      });
      ...
      let $filter = $(
        '<a href="#search-filter" class="button options" title="Filtre" tabindex="0" data-target="searchmenu" id="rcmbtn133" role="button"><span class="inner">Options</span></a>',
      ).click(() => {
    ...
    if (rcmail.env.task == 'mail') {
      ...
      rcmail.env.action == 'plugin.mel_moncompte'
    ) {
      $('#listes_ajouter')
        .click(function (e) {
      ...
      rcmail.env.task == 'settings' &&
      (rcmail.env.action == 'plugin.mel_resources_bal' ||
        rcmail.env.action == 'plugin.mel_resources_agendas' ||
        rcmail.env.action == 'plugin.mel_resources_contacts' ||
        rcmail.env.action == 'plugin.mel_resources_tasks')
    ) {
      $(document).on('click', '.showcontacts', function (e) {
        $('#list-contacts input#contactsearchbox')
          .removeAttr('disabled')
          .removeClass('ui-state-disabled');
        window.annuaireSelector =
          $('#input_hidden_isgroup').val() == 'false' ? 'person' : 'group';
        $('#listview-float-right').show();
        if ($('#annuaire-list li').length == 0) {
          rcmail.http_get(
            'mail/plugin.annuaire',
      ...
      $(document).on('click', '#list-contacts .close', function (e) {
        if ($('#listview-float-right').hide !== undefined)
          $('#listview-float-right').hide();
        else $('#listview-float-right').modal('hide');
        $('#annuaire-list li.added').removeClass('added');
      });
    }

    if (rcmail.env.searched_email_dn) {
      rcmail.annuaire_gototree_from_dn(
        rcmail.env.annuaire_source,
        rcmail.env.searched_email_dn,
      );
    }
  });
...
rcube_webmail.prototype.annuaire_gototree = function (obj, event) {
  ...
  this.annuaire_gototree_from_dn(source, dn);
};

// Node gototree
rcube_webmail.prototype.annuaire_gototree_from_dn = function (source, dn) {
  rcmail.http_get(
    'addressbook/plugin.annuaire',
    {
      _source: source,
...
rcube_webmail.prototype.annuaire_export = function () {
  this.goto_url(
    'export',
    { _cid: this.annuaire_list.get_selection(), _format: 'csv' },
    false,
    true,
  );
};
...
rcube_webmail.prototype.annuaire_node_select = function (node) {
  ...
  if (rcmail.env.task == 'addressbook') {
    ...
    !$('#rcmrow' + node_id).hasClass('added')
  ) {
    // find last focused field name
    if (!window.field) {
      window.field = $(this.env.focused_field).filter(':visible');
      window.field = window.field.length
        ? window.field.attr('id').replace('_', '')
        : 'to';
    }
    ...
    if (recipients.length && input.length) {
      ...
      if (oldval && !rx.test(oldval)) oldval += delim + ' ';
  ...
  const action = rcmail.env.annuaire_select_actions.onselect
    ? rcmail.env.annuaire_select_actions.onselect
    : null;
...
rcmail.addEventListener('responseafterplugin.annuaire', function (evt) {
  rcmail.enable_command(
    'search-create',
    evt.response.search_request ? true : false,
  );
...
rcube_webmail.prototype.annuaire_list_fill_list = function (
  parent_id,
  elements,
) {
  for (var i = 0; i < elements.length; i++) {
    ...
    if (
      ...
      elements[i].children[0]['html'] != '<span></span>'
    ) {
  ...
  rcmail.annuaire_list.container
    .find('div.treetoggle, .object')
    .each((i, e) => {
...
rcube_webmail.prototype.load_contact_annuaire = function (cid, action, framed) {
  ...
  } else if (framed) return false;

  if (action && (cid || action == 'add') && !this.drag_active) {
    if (this.env.group) url._gid = this.env.group;
...
rcube_webmail.prototype.annuaire_insert_saved_search = function (name, id) {
  var key = 'S' + id,
    link = $('<a>')
      ...
      .attr('rel', key)
      .click(function () {
        return rcmail.command('annuairelistsearch', id, this);
      })
      .html(name),
    prop = { name: name, id: id };

  rcmail.savedsearchlist.insert(
    { id: key, html: link, classes: ['contactsearch'] },
    null,
    'contactsearch',
  );
...
rcube_webmail.prototype.annuaire_set_loading_icon = function (load = true) {
  if (load) {
    if ($('.listsearchbox a.searchicon').length > 0)
      $('.listsearchbox a.searchicon')
        .attr('disabled', 'disabled')
        .addClass('disabled')
        .addClass('mel-before-remover')
        .html('<span class="spinner-grow"></span>');
  } else {
    if ($('.listsearchbox a.searchicon').length > 0)
      $('.listsearchbox a.searchicon')
        .removeAttr('disabled')
        .removeClass('disabled')
        .removeClass('mel-before-remover')
        .html('');
  }
};
