  ...
  $(document).ready(() => {
    try {
      function onDragStart(ev) {
        ...
        // On va chercher le texte propre, sans les éléments 'skip-on-drag'
        const sender =
          $row.find('.rcmContactAddress').first().text().trim() || 'Inconnu';
        const subject =
          $row.find('span.subject').last().text().trim() || '(Sans objet)';

        // 2. Gestion du multi-sélection
        const selectionCount = rcmail.message_list
        ...
        // On centre la souris sur le ghost (180/2 = 90)
        dt.setDragImage(container, 90, 40);

        // 5. Transmission des UIDs pour le drop
        const uids =
          (rcmail.message_list
            ? rcmail.message_list.get_selection().join(',')
            : rcmail.message_list.get_row_uid($row[0])) ||
          rcmail.message_list.get_row_uid($row[0]);
      ...
      function setupList() {
        ...
        $('#messagelist tbody tr').each((i, e) => {
          e = $(e);
          e.attr('draggable', true)
            .off('mouseup')
            .off('mousedown')
            .on('click', (ev) => {
              const uid = rcmail.message_list.get_row_uid(ev.currentTarget);
          ...
          e[0].addEventListener('dragstart', onDragStart);
          e[0].addEventListener('dragend', (ev) => {
            $('#folderlist-content .dragover').each((i, e) => {
        ...
        $(document)
          ...
          .on('drop', 'bnum-folder', (ev) => {
            ...
            if ($(ev.target).attr('is-virtual') === 'true') return;

            const folder = $(ev.target).attr('folder-id');

            rcmail.move_messages(folder, null, data);
          });
      }
