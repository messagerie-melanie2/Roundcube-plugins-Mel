function html_helper(option, html, optional_classes = '', attribs = null) {
  ...
  switch (option) {
    ...
    case html_helper.options.create_button:
      ...
      if (
        typeof optional_classes !== 'string' &&
        optional_classes !== null &&
        optional_classes !== undefined
      ) {
 ...
 */
html_helper.TasksAsync = async function (
  tabs,
  e = null,
  e_news = null,
  title = null,
) {
  let storage = await mel_metapage.Storage.check(
    mel_metapage.Storage.tasks,
  ).wait();

  if (html_helper.tasks_updates === undefined) {
    html_helper.tasks_updates = new Mel_Update(
      ...
      async () => {
        $('.html-tasks').each(async (i, e) => {
          ...
          e[0].outerHTML = await html_helper.TasksAsync(
            tabs,
            null,
            null,
            title,
...
html_helper.Tasks = function (
  ...
  let html = '';
  html +=
    '<div class="html-tasks" data-task-tabs="' +
    html_helper.JSON.stringify(tabs) +
    '" data-task-title="' +
    title +
    '">';
  ...
  html +=
    '<div style="margin:10px 0;"><div id="nb-waiting-task" class="nb-task wsp-task-classik tab-task mel-tab-content" style=""><span class="icon-mel-time roundbadge large clear"></span><span><span class="waiting-task"></span><span class="nb-waiting-task nb font-size-large">' +
    datas.length +
    '</span> tâches en cours</span></div></div>';
  ...
  for (let index = 0; index < datas.length; index++) {
    ...
    else html += '<div class=col-md-12></div>';
  ...
  if (e_news !== null) {
    ...
    } else e_news.addClass('hidden');
  }

  return html;
};

html_helper.CalendarsAsync = async function (
  config = {
          ...
          if (!$(element).hasClass('discussion-frame')) {
            element.contentWindow
              ...
              .each(async (i, calendar) => {
                ...
                if (
                  !(
                    config.add_day_navigation &&
                    moment(
                      calendar.find('.mm-agenda-date').data('current-date'),
                    )
                      .startOf('day')
                      .format() !== date.startOf('day').format()
                  )
                ) {
                  e[0].outerHTML = await html_helper.CalendarsAsync(
                    config,
                    null,
                    null,
                    date,
  ...
  if (
    moment().startOf('day').format() === moment(_date).startOf('day').format()
  )
    storage.where((x) => moment(x.end) > moment());

  return await html_helper.Calendars({
    datas: storage.toArray(),
    config: config,
    e: e,
    e_number: e_number,
    _date: _date,
...
html_helper.Calendars = async function ({
  datas,
  config = {
    add_day_navigation: false,
    add_create: false,
  ...
  const html_events = (
    await loadJsModule('mel_metapage', 'html_events', '/js/lib/html/')
  ).html_events;
  const html_li = (await loadJsModule('mel_metapage', 'html', '/js/lib/html/'))
    .html_li;

  let html = '';
  if (!get_only_body)
    html +=
      '<div class="html-calendar" data-config="' +
      html_helper.JSON.stringify(config) +
      '"  data-date="' +
      _date.format() +
      '">';
  if (config.add_day_navigation === true) {
    const count = Enumerable.from(datas)
      .where((x) => x.free_busy !== 'free')
      .count();
    let nav_click =
      "rcube_calendar.change_calendar_date($('.mm-agenda-date'), ¤¤¤)";
    html += '<div class="row">';
    html +=
      '<div class="col-2"><span class="icon-mel-calendar mm-agenda-icon"><span class="notif roundbadge lightgreen edited" ' +
      (typeof datas === 'string' || count === 0 ? 'style=display:none;' : '') +
      '>' +
      count +
      '</span></span></div>';
    html +=
      '<div class="col-6"><span class="mm-agenda-date">' +
      rcube_calendar.mel_metapage_misc.GetDate(_date) +
      '</span></div>';
    html += '<div class="col-4"><div class="row">';
    html +=
      '<div class="col-6"><button class="btn-mel-invisible btn-arrow btn btn-secondary" onclick="' +
      nav_click.replace('¤¤¤', '-1') +
      '"> <span class="icon-mel-arrow-left"><span class="sr-only">' +
      rcmail.gettext('last_day', 'mel_metapage') +
      '</span></span> </button></div>';
    html +=
      '<div class="col-6"><button class="btn-mel-invisible btn-arrow btn btn-secondary" onclick="' +
      nav_click.replace('¤¤¤', '1') +
      '"> <span class="icon-mel-arrow-right"><span class="sr-only">' +
      rcmail.gettext('next_day', 'mel_metapage') +
      '</span></span> </button></div>';
    html += '</div></div></div>';
  }
  if (!get_only_body) html += '<ul class="block-body ignore-bullet">';

  //let icon;
  if (typeof datas === 'string') html += '<div>' + datas + '</div>';
  else {
    if (datas.length > 0) {
      var $jquery_array = $('');
      let li;
      for (let index = 0; index < datas.length; index++) {
        ...
        li = new html_li({});
        new html_events(element, { 'data-ignore-date': true }, _date).appendTo(
          li,
        );
        li = li.generate();
      ...
      const storage = Enumerable.from(
        config.next_when_empty_today_function !== null &&
          typeof config.next_when_empty_today_function === 'function'
          ? config.next_when_empty_today_function(raw_storage)
          : raw_storage,
      ).where((x) => moment(x.start) > now);
      const storage_count = storage.count();
      if (storage_count > 0) {
        const storage_first = storage.first();
        const value =
          !!storage_first.value && !!storage_first.value[0]
            ? storage_first.value[0]
            : storage_first;
        ...
        html +=
          "<li>Pas d'évènements aujourd'hui ainsi que dans les 7 prochains jours !</li>";
    }
  }
  ...
  if (!get_only_body) {
    html += '</ul>';
    html += '</div>';
  } else html = `<ul class="ignore-bullet">${html}</ul>`;
  ...
  if (config.add_create === true) {
    ...
    if (!html.includes(data_text))
      html += html_helper(html_helper.options.create_button, data_text, {
        onclick:
          config.create_function === undefined ||
          config.create_function === null
            ? 'html_helper.Calendars.create()'
...
html_helper.Calendars.create = function (
  config = {
    date: moment(),
    selector: null,
  },
  ...
  let event = {
    start: moment(
      `${moment(date).format('YYYY-MM-DD')} ${moment().format('HH:mm')}`,
    ),
...
class mel_html {
  ...
  generate(additionnal_attribs = {}) {
    ...
    if (multi_balise && !!this.attribs[mel_html.ATTRIB_NO_MULTI_BALISE])
      multi_balise = false;

    let $html = $(
      `${CONST_BALISE_START}${this.tag} ${!multi_balise ? CONST_BALISE_CLOSE_END : CONST_BALISE_END}${multi_balise ? `${CONST_BALISE_CLOSE_START}${this.tag}${CONST_BALISE_END}` : EMPTY_STRING}`,
    );

    for (const iterator of Enumerable.from(this.attribs)
      .concat(additionnal_attribs)
      .where((x) => x === 0 || !!(x || null))) {
      switch (iterator.key) {
        case CONST_ATTRIB_CLASS:
          $html.addClass(
            Array.isArray(iterator.value)
              ? iterator.value.join(' ')
              : iterator.value,
          );
          break;
   ...
   */
  bind_events($element) {
    if (this.onclick.haveEvents())
      $element.on(CONST_EVENT_ACTION_CLICK, (event) => {
        this.onclick.call(event);
      });

    if (this.onkeydown.haveEvents())
      $element.on('keydown', (event) => {
        this.onkeydown.call(event);
      });

    if (this.onmouseover.haveEvents())
      $element.on('mouseover', (event) => {
        this.onmouseover.call(event);
      });

    if (this.onmouseout.haveEvents())
  ...
  addClass(classes) {
    ...
    if (!(this.attribs[CONST_ATTRIB_CLASS] || null))
      this.attribs[CONST_ATTRIB_CLASS] = [];
    else if (STRING === typeof this.attribs[CONST_ATTRIB_CLASS])
      this.attribs[CONST_ATTRIB_CLASS] = [this.attribs[CONST_ATTRIB_CLASS]];
  ...
  hasClass(html_class) {
    if (!!this.attribs && !!this.attribs[CONST_ATTRIB_CLASS]) {
      if (STRING === typeof this.attribs[CONST_ATTRIB_CLASS])
        return html_class === this.attribs[CONST_ATTRIB_CLASS];
  ...
  _getStyle() {
    if (!this.attribs['style'] || STRING === typeof this.attribs['style'])
      return this.attribs['style'] || EMPTY_STRING;
    else {
      let array = [];
      for (
        let keys = Object.keys(this.attribs['style']),
          index = 0,
          len = keys.length;
        index < len;
        ++index
      ...
      return EMPTY_STRING === array
        ? EMPTY_STRING
        : array + CONST_CSS_SEPARATOR;
    }
  }
...
class mel_html2 extends mel_html {
  ...
  _setup(contents = []) {
    if (!Array.isArray(contents)) {
      if (typeof contents === 'string')
        contents = [new mel_html('span', {}, contents)];
   ...
   */
  find_by_id(id) {
    return Enumerable.from(this.jcontents)
      .where((x) => x.attribs['id'] === id)
      .firstOrDefault();
  }
...
class amel_form_item extends mel_html {
  constructor(
    ...
    ignore_default_action = false,
  ) {
    super(tag, attribs, content);

    if (!ignore_default_action) {
      if (!this.attribs[CONST_ATTRIB_CLASS])
        this.attribs[CONST_ATTRIB_CLASS] = EMPTY_STRING;

      if (!this.attribs[CONST_ATTRIB_VALUE]) {
        this.attribs[CONST_ATTRIB_VALUE] =
          `${amel_form_item.CLASS_FORM_BASE} ${amel_form_item.CLASS_FORM_MEL}`;
      }

      if (
        !this.attribs[CONST_ATTRIB_CLASS].includes(
          amel_form_item.CLASS_FORM_BASE,
        )
      ) {
        this.attribs[CONST_ATTRIB_CLASS] +=
          ` ${amel_form_item.CLASS_FORM_BASE}`;
      ...
      if (
        !this.attribs[CONST_ATTRIB_CLASS].includes(
          amel_form_item.CLASS_FORM_MEL,
        )
      )
        this.attribs[CONST_ATTRIB_CLASS] += ` ${amel_form_item.CLASS_FORM_MEL}`;
    }
  ...
  toFloatingLabel(label) {
    let $label = new mel_html(
      CONST_HTML_DIV,
      { class: amel_form_item.CLASS_FORM_FLOATING },
      this.generate({ required: CONST_ATTRIB_REQUIRED }),
    ).generate();
    return $label.append(
      new mel_html(
        CONST_HTML_LABEL,
        { for: this.attribs[CONST_ATTRIB_ID] },
        label,
      ).generate(),
    );
  }
...
class mel_input extends amel_form_item {
  ...
  static togglePasswordShowed(element) {
    ...
    if ($input.length === 0)
      $input = top.$(`${CONST_JQUERY_SELECTOR_ID}${element.data(INPUT_DATA)}`);
  ...
  static floatingSetFocusClass(element, isOut = false) {
    ...
    if ($div.length === 0)
      $div = top.$(`${CONST_JQUERY_SELECTOR_ID}${element.data(DIV)}`);
...
class mel_password_with_button extends mel_password {
  ...
  generate(value, label = 'Mot de passe', additionnal_attribs = {}) {
    ...
    let $button = new mel_html(BALISE_DIV, { class: 'input-group-append' })
      .generate()
      .append(
        this.button.generate(button_config).append(this.button_span.generate()),
      );
    let $main = this.main.generate(main_config).append($input).append($button);
    let $label = new mel_html(BALISE_LABEL, label_config, label);

    return new mel_html(BALISE_DIV)
      .generate(return_config)
      .append($main)
      .append($label.generate());
  }
}
...
class mel_tab extends mel_button {
  ...
  generate(attribs = {}) {
    ...
    this.onclick.push((e) => {
      e = $(e.currentTarget);
      MelAsync.forof(
        $(`button.mel-html-tab.${e.data('tabnamespace')}`),
        async (iterator) => {
      ...
      MelAsync.forof(
        $(`mel-html-tabpanel.mel-html-tab.${e.data('tabnamespace')}`),
        async (iterator) => {
...
class mel_tablist extends mel_html {
  constructor(
    namespace,
    id,
    { attribs = {}, tabs = [], label = EMPTY_STRING },
  ) {
    super('div', attribs);
  ...
  generate(attribs = {}) {
    ...
    new mel_html('label', { for: this.id, class: 'sr-only' })
      .generate()
      .appendTo($tablist);
   ...
   */
  getSelectedTab() {
    return Enumerable.from(this.tabs)
      .where((x) => x.attribs['aria-selected'] === true)
      .firstOrDefault();
  }
...
class mel_tabpanel extends mel_html {
  constructor(
    namespace,
    tab,
    { attribs = {}, contents = EMPTY_STRING, jquery_content = null },
  ) {
...
class mel_tabs extends mel_html {
  ...
  generate({ attribs = {}, tablist_attribs = {} }) {
    ...
    for (
      let index = 0, len = this.contents.length, content;
      index < len;
      ++index
    ) {
      ...
      if (!!selected_tab && selected_tab.id === element.tab.id)
        content.css('display', '');
