      ...
      rcmail.addEventListener('responseafterlist', function (evt) {
        ...
        if (Enumerable.from(rcmail.options_labels).count() == 0) {
          $('#searchfilter option.labels').each((index, elem) => {
            const labelKey = Enumerable.from(elem.classList)
              .toArray()
              .join('.');
            ...
            if (!$(elem).hasClass(key)) $(elem).remove();
          });
        } else {
          ...
          if (
            rcmail.options_labels[`labels.${key}`] !== undefined &&
            rcmail.options_labels[`labels.${key}`]
          ) {
            for (const objectKey in rcmail.options_labels[`labels.${key}`]) {
              if (
                ...
                const element =
                  rcmail.options_labels[`labels.${key}`][objectKey];
...
function rcmail_tb_label_menu(p) {
  if (typeof rcmail_ui === 'undefined') rcmail_ui = UI;
  if (!rcmail_ui.check_tb_popup()) rcmail_ui.tb_label_popup_add();
  ...
  else rcmail_ui.show_popupmenu('tb_label_popup');
...
function rcm_tb_label_flag_toggle(flag_uids, toggle_label, onoff) {
  ...
  if (preview_frame.length) {
    tb_labels_for_message =
      preview_frame.get(0).contentWindow.tb_labels_for_message;
    headers_table = preview_frame
      .contents()
      .find('.message-partheaders table.headers-table');
    header_preview_all_table = preview_frame
      .contents()
      .find('.message-partheaders table.headers-table');
  }

  if (!rcmail.message_list && !headers_table) return;
  ...
  if (headers_table.length && flag_uids.length) {
    if (onoff == true) {
      ...
      if (header_preview_all_table.find('tr td.label').length > 0) {
        header_preview_all_table
          .find('tr td.label')
          .html(
            header_preview_all_table.find('tr td.label').html() +
              ', <span class="label_' +
              toggle_label +
              '">' +
              label_name +
        ...
        if (header_preview_all_table.length > 0) {
          header_preview_all_table.append(
            '<tr><td class="header-title">' +
              rcmail.gettext('mel_labels_sync.labels') +
              '</td><td class="header label"><span class="label_' +
              toggle_label.replace('~', '') +
              '">' +
              label_name +
              '</span></td></tr>',
          );
  ...
  jQuery.each(flag_uids, function (idx, uid) {
    ...
    if (onoff == true) {
      ...
      if (
        rowobj.find('td.labels').length === 0 &&
        rowobj.find('span.labels').length === 0
      )
      ...
      if (rowobj.find('td.labels').length > 0) {
        if (rowobj.find('td.labels').html() == '') {
          rowobj
            .find('td.labels')
            .html(
              `<span class="text_${'label_' + toggle_label.replace('~', '')}">${label_name}</span>`,
            );
        } else {
          rowobj
            .find('td.labels')
        ...
        if (rowobj.find('span.labels').html() == '') {
          ...
          rowobj
            .find('span.labels')
            .html(rowobj.find('span.labels').html() + ', ' + label_name);
        }
      }
    } else {
      // remove colors
...
function rcm_tb_label_get_selection() {
  var selection = rcmail.message_list
...
function rcm_tb_label_init_onclick(
  selector = '#tb_label_popup li a',
  closeAction = () => rcmail.hide_menu('tb_label_popup'),
) {
  if ($(selector).length) {
    $(selector).each(function () {
      ...
      $(this).click(function () {
        ...
        try {
          ...
          toggle_label = Enumerable.from(this.classList)
            .where((x) => x.includes('label_'))
            .first()
            .replace('label_', '');
        }
...
function rcmail_ctxm_label(command, el, pos) {
  ...
  if (!selection.length && !rcmail.env.uid) return;
...
function show_rcube_manage_labels() {
  var frame = $('<iframe>')
    .attr('id', 'managelabelsframe')
    .attr(
      'src',
      rcmail.url('settings/edit-prefs') + '&_section=labels&_framed=1',
    )
    .attr('frameborder', '0')
  ...
  jQuery.each(labels_list, function (index, value) {
    if (value) {
      ...
      if (_this.env.messages) {
        ...
        if (
          first_message.flags &&
          jQuery.inArray(toggle_label, first_message.flags.tb_labels) >= 0
        )
          first_toggle_mode = 'off';
        else first_toggle_mode = 'on';
      ...
      jQuery.each(selection, function (idx, uid) {
        ...
        if (!_this.env.messages) {
          if (first_toggle_mode == 'on') flag_uids.push(uid);
          else unflag_uids.push(uid);
          // make sure for unset all there is the single message id
          if (unset_all && unflag_uids.length == 0) unflag_uids.push(uid);
          return;
        }
        var message = _this.env.messages[uid];
        if (
          message.flags &&
          jQuery.inArray(toggle_label, message.flags.tb_labels) >= 0
        ) {
          if (first_toggle_mode == 'off') unflag_uids.push(uid);
        } else {
          if (first_toggle_mode == 'on') flag_uids.push(uid);
        }
      });

      if (unset_all) flag_uids = [];
...
rcube_webmail.prototype.mel_label_show_tooltip = async function ($parent) {
  ...
  if ($tooltip.length === 0) {
      ...
      $ul.append(
        $(
          ...
          .click((e) => {
            ...
            rcmail.triggerEvent('label.tooltip.click', {
              state: $target.is('.active'),
              $caller: $parent,
              $target,
            });
          })
          .data('mel-label', key)
          .addClass(`label_${key} txt important`)
          .on('mouseover', (e) => {
            let $target = $(e.target);
            const key = $target.data('mel-label');
    ...
    $parent
      ...
      .data('popup', 'mel-label-tooltip-dropdown');
...
$(document).ready(function () {
  ...
  $(document).keyup(function (e) {
    if (!$('input').is(':focus') && !$('textarea').is(':focus')) {
      ...
      if ((k > 47 && k < 58) || (k > 95 && k < 106)) {
        var label_no = k % 48;
        var cur_a = $('#tb_label_popup')
          .find('li.show.click' + label_no + ' a')
          .click();
      }
    }
  });
  ...
  if ($('#listoptions').length > 0) {
    $('#listoptions fieldset#listoptions-columns ul.proplist').append(
      '<li><label><input type="checkbox" name="list_col[]" value="labels" /> <span>' +
        rcmail.gettext('mel_labels_sync.labels') +
        '</span></label></li>',
    );
  }
  if ($('th#rcmlabels').length > 0) {
    $('th#rcmlabels span.labels').text(
      rcmail.gettext('mel_labels_sync.labels'),
    );
  }
      ...
      try {
        if (
          !mel_metapage.Functions.colors.kMel_LuminanceRatioAAA(
            mel_metapage.Functions.colors.kMel_extractRGB(val),
            mel_metapage.Functions.colors.kMel_extractRGB(textcolor),
          )
        )
          textcolor = 'black';
      } catch (error) {}
      ...
      rcmail.message_list.addEventListener('select', function (list) {
        ...
        $('div#tb_label_popup li a').each(function () {
          if ($(this).attr('id') != 'rcube_manage_labels') {
            ...
            if (selection.length == 0) $(this).removeClass('active');
  ...
  rcmail.addEventListener('listupdate', function () {
    if ($('td#rcmlabels').length > 0) {
      $('td#rcmlabels span.labels').text(
        rcmail.gettext('mel_labels_sync.labels'),
      );
    }
  });
  rcmail.addEventListener(
    'responseafterplugin.thunderbird_labels.update_list_labels',
    function (evt) {
  ...
  if (window.rcube_mail_ui) {
    rcube_mail_ui.prototype.tb_label_popup_add = function () {
      ...
      if (obj.length) this.popups.tb_label_popup.obj = obj;
      else delete this.popups.tb_label_popup;
    };
  }
  ...
  if (window.rcube_mail_ui) {
    rcube_mail_ui.prototype.check_tb_popup = function () {
      ...
      if (typeof this.popups === 'undefined') return true;
      if (this.popups.tb_label_popup) return true;
      else return false;
    };
  }
});
