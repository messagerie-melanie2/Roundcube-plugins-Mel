...
(function ($) {
  ...
  rcmail.addEventListener('init', function () {
    if (rcmail.env.mailbox && document.querySelector('bnum-folder')) {
      ...
      // 1. Initialisation manuelle du menu avec ta source HTML
      const menu = rcmail.contextmenu.init(
        {
          menu_name: 'folderlist',
          menu_source: '#mailboxoptionsmenu', // C'est ici qu'on lie ton HTML
          list_object: null,
        },
        {
          // On force l'activation des commandes pour éviter qu'elles soient grisées
          activate: function (p) {
            p.enabled = true;
            return p;
          },
      ...
      rcmail.contextmenu.init_folder = function (...args) {
        var [el, props, events] = args;

        if (!events) events = {};
        $('#rcm_folderlist').remove();
        rcmail.env.contextmenus['folderlist'] = undefined;
        let menu = rcmail.contextmenu.init(
          { menu_name: 'folderlist', list_object: null, ...props }, //$.extend({ menu_name: 'folderlist', list_object: null }, props),
          $.extend(
            {
              ...
              beforecommand: function (p) {
                if (
                  rcmail.env.context_menu_source_id != rcmail.env.mailbox &&
                  $.inArray(p.command, Array('expunge', 'purge')) >= 0
                ) {
                  rcmail[p.command + '_mailbox'](
                    rcmail.env.context_menu_source_id,
                  );
                  return { abort: true, result: true };
                } else if (
                  rcmail.env.context_menu_source_id != rcmail.env.mailbox &&
                  p.command == 'mark-all-read'
                ) {
      ...
      rcmail.addEventListener('menu-open', function (p) {
        ...
        rcmail.contextmenu.position ??= function (e, menu) {
          ...
          if (top + elem_height > win_height) {
            ...
            if (top < 0) top = Math.max(0, (win_height - elem_height) / 2);
          }
        ...
        $(`#${p.name} ul li a`).each((i, e_) => {
          ...
          for (const element of e.classList.values()) {
