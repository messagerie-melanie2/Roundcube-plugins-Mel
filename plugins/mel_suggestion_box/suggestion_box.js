...
if (window.rcmail) {
		...
		var tab = $('<span>').attr('id', 'settingstabpluginmel_suggestion_box')
	    	...
	    	.attr('href', rcmail.env.comm_path
	    			+ '&_action=plugin.mel_suggestion_box').attr('title', rcmail
	    					.gettext('mel_suggestion_box.showsuggestionbox')).html(rcmail
	    							.gettext('mel_suggestion_box.suggestionbox')).appendTo(tab);
	...
	rcmail.addEventListener('responseafterplugin.mel_suggestion_box_send', function(evt) {
		if (evt.response.ret) {
			...
			$('#mel_suggestion_box_option_box .sent_message').text(rcmail.gettext('mel_suggestion_box.suggestionboxmessagesent'));
		}
		else {
			rcmail.display_message(rcmail.gettext('mel_suggestion_box.suggestionboxmessagesenterror'), 'error');
		}
	});
}
...
rcube_webmail.prototype.send_suggestion_message = function() {
	if (!$('textarea#suggestion-text').val().length) {
		rcmail.display_message(rcmail.gettext('mel_suggestion_box.suggestionboxmessageempty'), 'error');
		return;
	}
	var lock = this.display_message(rcmail.gettext('mel_suggestion_box.suggestionboxmessagesend'), 'loading');
	this.http_post('plugin.mel_suggestion_box_send', {
