<?php
/**
 * Plugin Mél ownCloud/nextCloud
 *
 * plugin mel pour l'acces aux fichiers partagés
 *
...
class mel_owncloud extends rcube_plugin {
  /**
   *
   * @var string
   */
  ...
  function init() {
    ...
    $filter_ldap = $rcmail->config->get('roundcube_owncloud_filter_ldap', array());
    ...
    // Ajout du css
    $this->include_stylesheet($this->local_skin_path() . '/mel_owncloud.css');

    // ajout de la tache
    $this->register_task('owncloud');
    $this->add_button(array(
            'command' => 'owncloud',
            'class' => 'button-mel_owncloud',
            'classsel' => 'button-mel_owncloud button-selected',
            'innerclass' => 'button-inner',
            'label' => 'mel_owncloud.task'
    ), 'taskbar');

    // Si tache = owncloud, on charge l'onglet
  ...
  function action() {
    ...
    $rcmail->output->send('mel_owncloud.mel_owncloud');
  }
  /**
  ...
  function logout_after($args) {
    ...
    if ($this->is_internal()) {
      ...
      $owncloud_url = $rcmail->config->get('owncloud_external_url');
    }
    $rcmail->output->set_env('owncloud_url', $owncloud_url);
    // Appel le script de deconnexion du owncloud
  ...
  /**
   * Méthode pour se logger dans l'application de owncloud
   */
  private function login_owncloud() {
    ...
    // Appel le script de connexion du owncloud
